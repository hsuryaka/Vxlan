...
def parseKeyChainConfigs(log, args):
    ...
    arggrammar['key_life_time_end_infinite']='-type int'
...
class configTunnelEncryption():
    ...
    def configureKeyChain(self, node):
        ...
        if hasattr(ns,'key_life_time_end_infinite'):
            log.info("Inside the key_life_time_end_infinite")
            send_life_time_str += ' infinite '
            log.info(send_life_time_str)
        else:
