...
def parseGlobalAfIpv4Unicast( log, args ):
    ...
    arggrammar['default_metric']='-type int'
    ...
    arggrammar['allocate_label_all']='-type bool'
    arggrammar['allocate_label_rmap']='-type str'
    ...
    arggrammar['advertise_l2vpn']='-type str'
    arggrammar['advertise_l2vpn_type']='-type str'
    ns=parserutils_lib.argsToCommandOptions( args, arggrammar, log )
    return ns
...
def parseGlobalAfL2vpnEvpn( log, args ):
    ...
    arggrammar['retain_route_target']='-type bool'
                 ...
                 if 'address_family' in self.bgp_config_dict[node][as_no][vrf_name]:
                     ...
                     for family in self.bgp_config_dict[node][as_no][vrf_name]['address_family'].keys():
                       ...
                       if hasattr( ns, 'suppress_inactive') and ns.suppress_inactive:
                            cfg = cfg + '\n' + \
                              '''suppress-inactive'''
                       # Enable PIP for EVPN Vxlan
                       if hasattr(ns, 'advertise_pip') and ns.advertise_pip:
                            cfg = cfg + '\n' + \
                                '''advertise-pip'''
                       # Re-distribution
                       ...
                       if hasattr( ns, 'advertise_l2vpn' ) and ns.advertise_l2vpn is not None:
                                     cfg = cfg + '\n' + \
                                      '''advertise {0} {1}'''.format(ns.advertise_l2vpn, \
                                      ns.advertise_l2vpn_type)
                       if hasattr( ns, 'allocate_label_all') and ns.allocate_label_all is not None:
                                     cfg = cfg + '\n' + \
                                      '''allocate-label all''' 
                       if hasattr( ns, 'allocate_label_rmap') and ns.allocate_label_rmap is not None:
                                     cfg = cfg + '\n' + \
                                      '''allocate-label route-map {0}'''.format(ns.allocate_label_rmap) 


                            #network_addr=utils.incrementIpv4Address( network_addr, ns.network_step )
                       if hasattr(ns, 'retain_route_target') and ns.retain_route_target:
                                 cfg = cfg + '\n' + \
                   ...
                   for template in self.bgp_config_dict[node][as_no][vrf_name]['templates'].keys():
                     ...
                     if 'address_family' in self.bgp_config_dict[node][as_no][vrf_name]['templates'][template]:
                           for family in self.bgp_config_dict[node][as_no][vrf_name]['templates'][template]['address_family'].keys():
                                 ...
                                 if hasattr( afp, 'advertise_l2vpn' ) and afp.advertise_l2vpn is not None:
                                     cfg = cfg + '\n' + \
                                      '''advertise {0} {1}'''.format(afp.advertise_l2vpn, \
                                     afp.advertise_l2vpn_type)
