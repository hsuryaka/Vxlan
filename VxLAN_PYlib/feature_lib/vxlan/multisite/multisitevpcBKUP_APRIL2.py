...
parameters = {
    ...
    'routed_vlan' : 101,
    'routed_vni' : 90101,
    'routing_vlan_scale': 32,
    'ipv4_add' : '5.0.0.1',
    'ipv6_add' : '5::1',
    'mcast_group': '225.5.0.1',
    'mcast_group_scale' : 32,
    'vpc_po' : '101',
    'bgp_as_number' : '65001',
...
class common_setup(aetest.CommonSetup):
    ...
    def connect(self, testscript, testbed):
        ...
        #DeviceConnect(uut):
        #result = pcall(DeviceConnect,uut=tuple(uut_list))
        #if not result:
        #    log.info('DeviceConnect Failed ')
    ...
    def configureTgn(self, testscript, testbed):
        ...
        global port_handle_bgw2site3,test_port_list,rate_list,tgen, port_handle_sw1site2,port_handle_sw1site3,port_handle_dci1,\
...
class TC0001_vxlan_configs(aetest.Testcase):
    ...
    def vxlan_configs_site1(self, testscript, testbed):
        ...
        pcall(vxlanConfigureAuto222,\
            #uut=(site1_bgw_uut_list[0],site1_leaf_uut_list[0],site1_leaf_uut_list[1],\
            ...
            #mcast_group_scale =(mcast_group_scale,mcast_group_scale,mcast_group_scale,mcast_group_scale,mcast_group_scale,mcast_group_scale,mcast_group_scale,mcast_group_scale,mcast_group_scale,mcast_group_scale,mcast_group_scale),\
...
class TC0006_vxlan_ms_configs(aetest.Testcase):
    ...
    def multiSiteEnable(self):
        ...
        try:
            ...
            vni=(101001,101001,101001,101001,101001),scale=(vlan_vni_scale,vlan_vni_scale,vlan_vni_scale,vlan_vni_scale,vlan_vni_scale))
...
class TC0007_vxlan_ms_traffic(aetest.Testcase):
    ...
    def setup(self):
        ...
        #SpirentHostBidirStreamType5(port_hdl1,port_hdl2,vlan1,vlan2,ip1,ip2,gw1,gw2,rate_pps)

        #StreamCreate(port_handle_bgw2site2,'22.22.22.99','5.2.0.99','22.22.22.1',200000)
        #StreamCreate(port_handle_bgw2site3,'33.33.33.99','5.3.0.99','33.33.33.1',200000)
...
''' 
class TC113_vxlan_ms_vPC_bgw_bringup(aetest.Testcase):
    ###    This is description for my tecase two

    @aetest.setup
    def setup(self): 
        if not traffictest1(test_port_list,rate_list):
            self.failed(goto=['common_cleanup'])
...
class TC114_vxlan_ms_vPC_bgw_traffic_to_vpc(aetest.Testcase):
    ###    This is description for my tecase two

    @aetest.setup
    def setup(self):
        if not setupTrafficTest(test_port_list,rate_list,bgw_uut_list):
            self.failed(goto=['common_cleanup'])
    ...
    def trafficSwitchover(self, testscript, testbed):
        ...
        site2_sw_uut_list[0].configure(add_102.format(vlan_range=vlan_range))
        for uut in site2_bgw_uut_list:
            uut.configure(add_102.format(vlan_range=vlan_range))
    ...
    def vxlan_traffic_test_all(self):
        if not traffictest1(test_port_list,rate_list):
            self.failed(goto=['common_cleanup'])
        log.info(' MS VPC --- saveConfigtoBootflash1  ')
...
class TC1141_vxlan_ms_same_Loopback_addresses_on_Leafs(aetest.Testcase):
    ...
    def loopChange(self, testscript, testbed):
        ...
        loAd01 = site1_bgw_uut_list[0].execute('show running-config interface loopback0 | be loopback0')
        loAd02 = site1_bgw_uut_list[1].execute('show running-config interface loopback0 | be loopback0')

        loAd11 = site2_bgw_uut_list[0].execute('show running-config interface loopback0 | be loopback0')
        loAd12 = site2_bgw_uut_list[1].execute('show running-config interface loopback0 | be loopback0')
        ...
        site1_bgw_uut_list[0].configure(loAd11)
        site1_bgw_uut_list[1].configure(loAd12)
        ...
        site1_bgw_uut_list[0].configure(loAd01)
        site1_bgw_uut_list[1].configure(loAd02)
...
class TC114a_vxlan_ms_vPC_mac_move_3_1(aetest.Testcase):
    ...
    def ConsistencyChecker(self, testscript, testbed):
        if not pcall(vxlanMsiteCChecker,uut=tuple(bgw_uut_list)):
            self.failed()
...
class TC127_vxlan_ms_vPC_bgw_fabric_link_failover1(aetest.Testcase):
   ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        if not AllTrafficTestMsite(port_handle_sw1site2,port_handle_sw1site3,rate,int(pps),orphan_handle_list):
...
class TC154_vxlan_ms_vPC_bgw_clear_ospf_Neigh(aetest.Testcase):
    ###    This is description for my tecase two
...
class TC160_vxlan_ms_vPC_bgw_Spine_Clear_OSPF(aetest.Testcase):
    ###    This is description for my tecase two
...
class TC173_vxlan_ms_vPC_bgw_bg1_isolation_nve_shut(aetest.Testcase): 
    ...
    def TriggerNveShutBgw1(self, testscript, testbed):
        ...
        if not AllTrafficTestMsite(port_handle_sw1site2,port_handle_sw1site3,rate,int(pps),[port_handle_sw1site1,port_handle_bgw2site2]):
...
class TC174_vxlan_ms_vPC_bgw_bg2_isolation_nve_shut(aetest.Testcase): 
    ...
    def TriggerNveShutBgw1(self, testscript, testbed):
        ...
        if not AllTrafficTestMsite(port_handle_sw1site2,port_handle_sw1site3,rate,int(pps),[port_handle_sw1site1,port_handle_bgw1site2]):
            log.info(banner(" failed for SD @ 1 - Additional Countdown(100) total 175"))
            countdown(50)
            if not AllTrafficTestMsite(port_handle_sw1site2,port_handle_sw1site3,rate,int(pps),[port_handle_sw1site1,port_handle_bgw1site2]):
                log.info(banner(" failed for SD - Additional Countdown(100) total 200"))
...
class TC200_vxlan_ms_vPC_bgw_to_anycast_GW_convert(aetest.Testcase):
    ###    This is description for my tecase two
