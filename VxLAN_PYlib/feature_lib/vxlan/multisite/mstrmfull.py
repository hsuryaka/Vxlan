...
class common_setup(aetest.CommonSetup):
    ...
    def testbed_init(self, testscript, testbed):
        ...
        global ir_mode,orphan_handle_list_sa,vtep_emulation_spirent,vtep_scale,igp,uut_list,leaf_uut_list,spine_uut_list ,bgw_uut_list,sw_uut_list , l3_uut_list,l3_site_uut_list,\
    ...
    def igp_configure(self):     
        ...
        try:
            ...
            igp=(igp,igp,igp,igp,igp,igp,igp,igp,igp,igp))
        except:
    ...
    def configureTgn(self, testscript, testbed):
        ...
        orphan_handle_list_anycast_bgw

        try:
            port_handle = ConnectSpirent(labserver_ip,tgn_ip,[tgn1_dci1_intf1,tgn1_sw1site2_intf1,tgn1_sw1site3_intf1,\
            tgn1_bgw1site1_intf1,tgn1_bgw1site2_intf1,tgn1_sw1site1_intf1,tgn1_bgw1site3_intf1,tgn1_bgw2site3_intf1])
        except:
        ...
        orphan_handle_list_anycast_bgw = [port_handle_sw1site1]
...
class TC0001_vxlan_configs(aetest.Testcase):
    ...
    def vxlan_configs(self, testscript, testbed):
        ...
        try:
            pcall(vxlanConfigure,uut=(site1_bgw_uut_list[0],site1_leaf_uut_list[0],\
            site2_bgw_uut_list[0],site2_leaf_uut_list[0],\
            site3_bgw_uut_list[0],site3_bgw_uut_list[1],site3_leaf_uut_list[0]),\
            l2_scale =(vlan_vni_scale,vlan_vni_scale,vlan_vni_scale,vlan_vni_scale,vlan_vni_scale,vlan_vni_scale,vlan_vni_scale),\
            l3_scale =( routing_vlan_scale,routing_vlan_scale,routing_vlan_scale,routing_vlan_scale,routing_vlan_scale,routing_vlan_scale,routing_vlan_scale),\
            mode=(ir_mode,ir_mode,ir_mode,ir_mode,ir_mode,ir_mode,ir_mode),\
            as_num=('65001','65001','65002','65002','65003','65003','65003'))
    ...
    def dciEbgpevpn4Bringup(self):
        ...
        try:
            ...
            pcall(multisitebgwbgpEvpn,uut=tuple(bgw_uut_list),\
                as_num=('65001','65002','65003','65003'))
...
class TC0006_vxlan_multisite_configs(aetest.Testcase):
    ...
    def multiSiteEnable(self):
        ...
        try:
            ...
            vni=(vni,vni,vni,vni),scale=(vlan_vni_scale,vlan_vni_scale,vlan_vni_scale,vlan_vni_scale))
...
'''  
class TC007_vxlan_multisite_trm_configs(aetest.Testcase):
    ###    This is de  scription for my testcase two
    ...
    @aetest.test
    def trmenable(self):
        #pcall(trmEnablebgw,uut=tuple(bgw_uut_list)) 
        #pcall(trmEnablevtep,uut=tuple(leaf_uut_list))
        #pcall(trmallEnabledci,uut=tuple(dci_uut_list)) 
        #pcall(trmEnablespine,uut=tuple(spine_uut_list)) 
        pcall(unShutall,uut=tuple(uut_list))

    @aetest.test
    def trmCLIcheck(self):
        pass
...
class TC0007_vxlan_ms_traffic(aetest.Testcase):
    ...
    def vxlan_traffic_test_all(self):
        ...
        if not AllTrafficTest(port_handle_sw1site2,port_handle_sw1site3,rate,int(pps),orphan_handle_list_anycast_bgw):
            self.failed(goto=['common_cleanup'])
...
class TC0007_vxlan_trm_traffic(aetest.Testcase):
    ###    This is description for my testcase two
    ...
    def setup(self):
        ...
        log.info(banner("Finding the IP address"))
        ip_sa1=str(ip_address(find_svi_ip222(site1_leaf_uut_list[0],vlan_start))+10)
        ip_sa2=str(ip_address(ip_sa1)+256)
        ip_sa11=str(ip_address(ip_sa1)+40)
        ip_sa22=str(ip_address(ip_sa2)+40)
                
        log.info(banner("----Generating mcast flood traffic----"))
        test1= trmTrafficGeneratorScale(port_handle_sw1site2,vlan_start,ip_sa1,'239.2.2.2',rate,str(vlan_vni_scale))
        test2= trmTrafficGeneratorScale(port_handle_sw1site3,vlan_start,ip_sa2,'239.3.3.3',rate,str(vlan_vni_scale))
        

    @aetest.test
    def trmtraffictest(self, testscript, testbed):
        log.info(banner("Starting Traffic and counting 120 seconds"))
        log.info(banner("ARP for all streams"))
        for i in range(1,3):
            doarp = sth.arp_control(arp_target='allstream',arpnd_report_retrieve='1')
...
class TC008_vxlan_multisite_bgw_sa_l2fm_restart(aetest.Testcase): 
   ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        if not AllTrafficTest(port_handle_sw1site2,port_handle_sw1site3,rate,int(pps),orphan_handle_list_sa):
