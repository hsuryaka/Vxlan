...
class common_setup(aetest.CommonSetup):
    ...
    def testbed_init(self, testscript, testbed):
        global filename_bgw_leaf_list_trm,test_issu,issu_image,filename_list,filename,ext_rcv_rate,tgn1_ext1site1_intf1,port_handle_ext1site1,test_vrf_ext,load_from_bootflash,test_l3_vni_scale,traffic_profile,vtep_emulation_msite_spirent,rateFull,rateOrphan,site1_group1,site2_group1,site3_group1,ir_mode,vtep_emulation_spirent,vtep_scale,\
        ...
        filename='JUNE5ib_mstrm_l2_'+str(vlan_vni_scale)+'_l3_'+str(routing_vlan_scale)+'_mc_'+str(mcast_group_scale)
        ...
        filename_bgw_leaf_list_trm = [filename,filename,filename,filename,filename,filename,filename,filename]
    ...
    def checkbringup(self, testscript, testbed):
        if 'yes' in load_from_bootflash:
            ...
            pcall(loadmstrm256,uut=tuple(uut_list)) 
...
class TC00011_vxlan_trm_mcast_traffic_enable(aetest.Testcase):
    ...
    def setup(self):
        ...
        log.info(" ++++++++++  filename_list is %r",filename_list)        
        ...
        if 'full' in traffic_profile:    
            ...
            vxlantrafficSetupfullScaled(site1_bgw_uut_list[0],port_handle_sw1site1,port_handle_sw1site2,vlan_start,vlan_vni_scale,rate,test_l3_vni_scale)
...
class TC00000333_vxlan_ms_add_rem_dup_ip(aetest.Testcase):
    ...
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
            if not rateTest(port_hdl,exp_rate):
                log.info('rateTest failed for %r',port_hdl)
                pcall(reload_with_valid_config,uut=tuple(bgw_uut_list+leaf_uut_list),\
               	filename=tuple(filename_bgw_leaf_list_trm))
                countdown(500)
                if not rateTest(port_hdl,exp_rate):
                    log.info('rateTest failed for %r',port_hdl)
                    self.failed(goto=['common_cleanup'])
...
class TC00022_vxlan_ms_trm_clear_ip_mroute_vrf(aetest.Testcase):
    ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
    ...
    @aetest.test
    def clear_ip_mroute_vr(self, testscript, testbed):
        log.info(banner("Starting clear_ip_mroute_vr "))

        for uut in site1_bgw_uut_list:
            for i in range(1,5):     
                uut.execute("clear ip mroute * vrf all") 
...
class TC000201_vxlan_ms_trm_remove_add_multisite_under_l3_vni(aetest.Testcase):
    ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
    ...
    @aetest.test
    def remove_add_multisite_under_l3_vni(self, testscript, testbed):
        log.info(banner("Starting ipRemoveAdd "))

        if not pcall(RemovAddMsiteConfig,uut=tuple(bgw_uut_list)):
            self.failed()            
...
class TC0000033311_vxlan_ms_issu(aetest.Testcase):
    ###    This is description for my tecase two
