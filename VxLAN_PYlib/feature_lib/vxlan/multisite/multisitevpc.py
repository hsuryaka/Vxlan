...
class common_setup(aetest.CommonSetup):
    ...
    def testbed_init(self, testscript, testbed):
        ...
        filename='imr5_msvpc_l2_'+str(vlan_vni_scale)+'_l3_'+str(routing_vlan_scale)+'_mc_'+str(mcast_group_scale)
     
        filename_bgw_list = [filename,filename,filename,filename,filename]
    ...
    def checkbringup(self, testscript, testbed):
        for uut in bgw_uut_list+leaf_uut_list:
            uut.execute("show version")
        ...
        if 'yes' in skip_Setup:
            log.info('+++++Skipping Setup -skip_Setup- TC00001_configureTgn +++++')
            goto=['TC00001_configureTgn']
 
            countdown(60)

        else:
            log.info('+++++NO skip_Setup+++++') 
    ...
    def precleanup(self, testscript, testbed):
        ...
        #if not pcall(reloaduut,uut=tuple(bgw_uut_list)):
        #    self.failed(goto=['common_cleanup']) 
...
class TC113_vxlan_ms_vPC_bgw_bringup(aetest.Testcase):
    ...
    def vpc_configure(self):
        ...
        log.info(banner("Configuring IGP over Channel/VPC in BGW devices"))
        pcall(mctsviConfigure,uut=tuple(site2_bgw_uut_list),igp=(igp,igp))

        log.info(banner("Configuring IGP over Channel/VPC in BGW devices"))
        pcall(pip_configure,uut=tuple(site2_bgw_uut_list+site3_bgw_uut_list))
    ...
    def trafficSwitchover(self, testscript, testbed):
        ...
        site2_sw_uut_list[0].configure(add_rem_vlan.format(intf='po102',vlan_range=vlan_range))
        site2_sw_uut_list[0].configure(add_rem_vlan.format(intf='po101',vlan_range='none'))
        for uut in site2_bgw_uut_list:
            uut.configure(add_rem_vlan.format(intf='po102',vlan_range=vlan_range))
        for uut in site2_leaf_uut_list:
            uut.configure(add_rem_vlan.format(intf='po101',vlan_range='none'))
...
class TC00001_configureTgn(aetest.Testcase):
    ...
    def configureTgn(self, testscript, testbed):
        ...
        port_handle = ConnectSpirent(labserver_ip,tgn_ip,[tgn1_dci1_intf1,tgn1_sw1site2_intf1,tgn1_sw1site3_intf1,\
            tgn1_bgw1site2_intf1,tgn1_bgw2site2_intf1,tgn1_sw1site1_intf1,tgn1_bgw1site3_intf1,tgn1_bgw2site3_intf1])
...
class TC0007_vxlan_ms_vpc_traffic(aetest.Testcase):
    ###    This is description for my testcase two
    ...
    def nxosvxlancontrolplanebgw(self, testscript, testbed): 
        op1 = bgw_uut_list[2].execute('sh mac add dynamic vlan 1001 | inc Po102')
        for line in op1.splitlines():
            if line:
                if 'dynamic  0         F      F    Po102' in line:
                    mac1 = line.split()[2] 
                    for uut in bgw_uut_list:   
                        if not nxosVxlanEvpnCheck(uut,mac=mac1):
    ...
    def vxlan_traffic_test_all(self):
        ...
        if not traffictest1(test_port_list,rate_list):
            pcall(reload_with_valid_config,uut=tuple(bgw_uut_list),\
            filename=tuple(filename_bgw_list))
            countdown(900)
            if not traffictest1(test_port_list,rate_list):              
                log.info('rateTest failed for ')
                #self.failed()
                self.failed(goto=['common_cleanup'])                
...
class TC000001_vxlan_ms_Peer_type_fabric_external_rem_add3(aetest.Testcase):
    ###    This is description for my tecase two
    ...
    def ConsistencyChecker(self, testscript, testbed):
        if not vxlanMsiteCCheckerAll(site3_bgw_uut_list):
            self.failed()
...
class TC00000111_vxlan_ms_bgp_remove_Configure_replace(aetest.Testcase):
    ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        if not traffictest1(test_port_list,rate_list):
    ...
    @aetest.test
    def mstrmConfigReplacebgp(self, testscript, testbed):
...
class TC00000111_vxlan_ms_pim_remove_Configure_replace(aetest.Testcase):
    ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        if not traffictest1(test_port_list,rate_list):
    ...
    @aetest.test
    def mstrmConfigReplacepim(self, testscript, testbed):
...
class TC000001_vxlan_ms_nv_overlay_remove_Configure_replace(aetest.Testcase):
    ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        if not traffictest1(test_port_list,rate_list):
    ...
    @aetest.test
    def mstrmConfigReplacenv(self, testscript, testbed):
...
class TC0000333_vxlan_ms_Peer_type_fabric_external_rem_add2(aetest.Testcase):
    ###    This is description for my tecase two
...
class TC144_vxlan_ms_vPC_bgw_restart_bgp(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    @aetest.test
    def restart_bgp(self, testscript, testbed):
        log.info(banner("Starting restart ospf UNDERLAY @ 8"))
        for uut in site2_bgw_uut_list:
...
class TC145_vxlan_ms_vPC_bgw_restart_pim(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    @aetest.test
    def restart_pim(self, testscript, testbed):
        log.info(banner("Starting restart ospf UNDERLAY @ 8"))
        for uut in site2_bgw_uut_list:
...
class TC146_vxlan_ms_vPC_bgw_restart_igmp(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    @aetest.test
    def restart_igmp(self, testscript, testbed):
        log.info(banner("Starting restart ospf UNDERLAY @ 8"))
        for uut in site2_bgw_uut_list:
...
class TC147_vxlan_ms_vPC_bgw_restart_mld(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    @aetest.test
    def restart_mld(self, testscript, testbed):
        log.info(banner("Starting restart ospf UNDERLAY @ 8"))
        for uut in site2_bgw_uut_list:
