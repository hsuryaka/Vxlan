...
"""
1 stream brings 4000 pps on rx
...
igp = 'isis'
vtep_emulation_site1_spirent = 'no'
vtep_emulation_msite_spirent = 'no'
dci_vtep_scale = 14
trm_test_l3_vlan_scale = 1 # for trm

ext_rcv_rate = 0
#test_rate = int(trm_test_l3_vlan_scale)*8000*int(group_scale_per_port)

#traffic_profile = 'trm' # 'full'
traffic_profile = 'full'
test_l3_vni_scale = 4 # for vxlan
...
class common_setup(aetest.CommonSetup):
    ...
    def testbed_init(self, testscript, testbed):
        global dci_vtep_scale,site1_vtep_scale,vtep_emulation_site1_spirent,rate_list_trm_only,rate_list_non_trm_only,filename_list,filename,ext_rcv_rate,tgn1_ext1site1_intf1,port_handle_ext1site1,test_vrf_ext,load_from_bootflash,test_l3_vni_scale,traffic_profile,vtep_emulation_msite_spirent,rateFull,rateOrphan,site1_group1,site2_group1,site3_group1,ir_mode,vtep_emulation_spirent,vtep_scale,\
        igp,uut_list,leaf_uut_list,spine_uut_list,bgw_uut_list,sw_uut_list,l3_uut_list,l3_site_uut_list,port_handle_bgw1site1,\
        ...
        site2_uut_list,site2_leaf_uut_list,site2_spine_uut_list,site2_bgw_uut_list,site2_sw_uut_list,trm_test_l3_vlan_scale,\
        site3_uut_list,site3_leaf_uut_list,site3_spine_uut_list,site3_bgw_uut_list,site3_sw_uut_list,dci_uut_list,ext_uut_list,site1_ext_uut_list,\
        ...
        ext_uut_list = [] ; site1_ext_uut_list = []
        ...
        tgn1_ext1site1_intf1 = testbed.devices['tgn1'].interfaces['tgn1_ext1site1_intf1'].intf
        ...
        for device in testbed.devices:
            if 'dci' in (testbed.devices[device].alias):
                ...
                elif 'ext' in (testbed.devices[device].alias):
                    site1_ext_uut_list.append(testbed.devices[device])
                    ext_uut_list.append(testbed.devices[device])
        ...
        vlan_per_vrf=vlan_vni_scale/routing_vlan_scale
        #ext_mcast_rate = vlan_per_vrf*10000

        count10 = int(vlan_vni_scale/routing_vlan_scale)
        if count10 == 4:
            value1 = 8000
        elif count10 == 8:
            value1 = 16000
        ...
        test_rate = int(trm_test_l3_vlan_scale)*value1*int(group_scale_per_port)
        log.info('test_rate is %r',test_rate)
        filename='vsg_ib_mstrm_'+str(vlan_vni_scale)+'_'+str(routing_vlan_scale)+'_'+str(mcast_group_scale)

        filename_list=[filename,filename,filename,filename,filename,filename,filename,filename,\
        filename,filename,filename,filename,filename,filename,filename,filename,filename]

        if 'full' in traffic_profile:
            #rateFull = int(rate)*4+int(test_rate)+int(ext_mcast_rate)  
            #rateOrphan = int(rate)*2+int(test_rate)+int(ext_mcast_rate)  
        ...
        rate_list_trm_only = [test_rate,test_rate,test_rate]
        rate_list_non_trm_only = [int(rate)*4 ,int(rate)*4 ,int(rate)*2]


    @aetest.subsection
    def connect(self, testscript, testbed):
        if not ConnectAll(uut_list):
        #if not ConnectAll(dci_uut_list+spine_uut_list+bgw_uut_list):
            self.failed(goto=['common_cleanup'])

        #for uut in uut_list:
        #    uut.configure('copy run bootflash:vsg-mstrm-may-8')
    ...
    def checkbringup(self, testscript, testbed):
        ...
        #pcall(loadmstrmvsg,uut=tuple(uut_list)) 
    ...
    def precleanup(self, testscript, testbed):
        ...
        #if not pcall(reloaduut,uut=tuple(bgw_uut_list+leaf_uut_list)):
        #    self.failed(goto=['common_cleanup']) 
    ...
    def check_nve_vni_state(self):
        for uut in leaf_uut_list+bgw_uut_list:
            ...
            test1 = vniCheck(uut,vlan_vni_scale,routing_vlan_scale)
            if not test1:
                self.failed(goto=['common_cleanup']) 
...
class TC00004_Vxlan_Consistency_check(aetest.Testcase):
    ...
    def ConsistencyChecker(self, testscript, testbed):
        if not pcall(vxlanMsiteCChecker,uut=tuple(site1_bgw_uut_list)):
            self.failed()
...
class TC00006_vxlan_ms_configs(aetest.Testcase):
    ...
    def multiSiteAnycastEnable(self):
        try:
            if not anycastBgwBgpConfgure(site1_bgw_uut_list):
                self.failed(goto=['common_cleanup']) 
            if not anycastBgwBgpConfgure(site2_bgw_uut_list):
                self.failed(goto=['common_cleanup'])             
        except ValueError:
...
class TC00008_vxlan_ms_trm_configs(aetest.Testcase):
    ...
    def check2(self):        
        if 'yes' in load_from_bootflash:
            for uut in uut_list:
                uut.configure('delete bootflash:ms_trm_1023_l2_256_l3_feb_20 no-prompt')
            for uut in uut_list:
                uut.configure('copy run bootflash:ms_trm_1023_l2_256_l3_feb_20',timeout=120)
            for uut in uut_list:
...
class TC00001_external_connectivity(aetest.Testcase):
    ###    This is de  scription for my testcase two
    ...
    @aetest.test
    def externalConnection(self):
        if not vrfLitebgwExtconnection(site1_bgw_uut_list[0],site1_ext_uut_list[0]):
            self.failed()

    @aetest.test
    def externalRp(self):
        if not pcall(mstrmExtRpConfig,uut=tuple(bgw_uut_list+leaf_uut_list)):
            self.failed() 
...
class TC00001_configureTgn(aetest.Testcase):
    ...
    def configureTgn(self, testscript, testbed):
        ...
        global tgen,port_handle_ext1site1,tgn1_ext1site1_intf1,port_handle_sw1site1,port_handle_sw1site2,port_handle_sw1site3,port_handle_dci1,orphan_handle_list_sa,\
        port_handle_bgw1site2,orphan_handle_list,port_handle_list,orphan_handle_list_sa,port_handle_bgw1site1,\
        orphan_handle_list_anycast_bgw,tgn1_spine2site1_intf1,port_handle_spine2site1,trm_port_handle_list,\
        site1_group1,site2_group1,site3_group1,test_rate,trm_test_l3_vlan_scale,rate_list_trm


        try:
            port_handle = ConnectSpirent(labserver_ip,tgn_ip,[tgn1_ext1site1_intf1,tgn1_dci1_intf1,tgn1_sw1site2_intf1,tgn1_sw1site3_intf1,\
            tgn1_bgw1site1_intf1,tgn1_bgw1site2_intf1,tgn1_sw1site1_intf1,tgn1_spine2site1_intf1])

            #port_handle = ConnectSpirent(labserver_ip,tgn_ip,[tgn1_spine2site1_intf1,tgn1_dci1_intf1])
        except:
        ...
        port_handle_ext1site1 = port_handle[tgn1_ext1site1_intf1]  
        ...
        #if 'yes' in load_from_bootflash:
        #    log.info('+++++load_from_bootflash+++++')
        #    pcall(loadmstrm,uut=tuple(uut_list)) 
        #    goto=['TC00011_vxlan_trm_mcast_traffic_enable']

 
class TC00001_vtepEmulationDCI(aetest.Testcase):
    ###    This is de  scription for my testcase two
    ...
    def vtepEmulationBgp(self):
        ...
        if 'yes' in vtep_emulation_msite_spirent:            
            ...
            try:
                vtepEmulationBgpConfSpine(dci_uut_list[0],dci_vtep_scale)
            except Exception as e:
                log.info('------Print error------')
                log.info('error is %r',e)
                log.info('------Print error done------')
                log.info('vtepEmulationBgpConfSpine Failed')    
                self.failed()  
    ...
    def vtepEmulationMsite(self):
        ...
        if 'yes' in vtep_emulation_msite_spirent:        
            ...
            log.info("in  vtepEmulation")
            try:
                if not SpirentVtepEmulationDCI(uut,port_handle_dci1,dci_vtep_scale):
                    self.failed()                
            except Exception as e:
                log.info('------Print error------')
                log.info('error is %r',e)
...
class TC00001_vtepEmulationBgpSpineSite1(aetest.Testcase):
    ###    This is de  scription for my testcase two
    ...
    def vtepEmulationBgp(self):
        """ testcase clean up """
        if 'yes' in vtep_emulation_site1_spirent:        
            log.info("in  vtepEmulation BGP")
            try:
                vtepEmulationBgpConfSpine(site1_spine_uut_list[1],site1_vtep_scale)
            except Exception as e:
                log.info('------Print error------')
                log.info('error is %r',e)
                log.info('------Print error done------')
    ...
    def vtepEmulationMsite(self):
        """ testcase clean up """
        if 'yes' in vtep_emulation_site1_spirent:   
            uut = site1_spine_uut_list[1]     
            log.info("in  vtepEmulation")
            try:
                if not SpirentVtepEmulationSpine(uut,port_handle_spine2site1,site1_vtep_scale):
                    self.failed()
            except Exception as e:
                log.info('------Print error------')
                log.info('error is %r',e)
...
class TC00001_vtep_peer_count_check(aetest.Testcase):
    ###    This is de  scription for my testcase two
    ...
    @aetest.test
    def vtep_peer_count_check(self):
        """ testcase clean up """
        fail_list = []
        for uut in site1_bgw_uut_list:
            op = uut.execute('show icam scale vxlan | inc VTEP')
            for line in op.splitlines():
                if 'VTEP' in line:
                    count = int(line.split()[4])
                    if count == site1_vtep_scale:
                        log.info('vtep count equel to site1_vtep_scale')     
                    elif count > site1_vtep_scale:
                        log.info('vtep count > site1_vtep_scale')    
                    elif count < site1_vtep_scale:
                        log.info('vtep count < site1_vtep_scale')
            ...
            count2 = uut.execute("show nve peers | count")
            if int(count2) < site1_vtep_scale:
                log.info('vtep count < site1_vtep_scale in CLI')
                fail_list.append('fail') 
...
class TC00001_vni_count_check_icam(aetest.Testcase):
    ###    This is de  scription for my testcase two
    ...
    @aetest.test
    def vni_count_check(self):
        """ testcase clean up """
        fail_list = []
        for uut in site1_bgw_uut_list:
            op = uut.execute('show icam scale vxlan | inc Anycast')
            for line in op.splitlines():
                if 'Anycast' in line:
                    count = int(line.split()[7])
                    if count == vlan_vni_scale:
                        log.info('vni count equel to vlan_vni_scale')     
                    elif count > vlan_vni_scale:
                        log.info('vni count > vlan_vni_scale')    
                    elif count < vlan_vni_scale:
                        log.info('vni count < vlan_vni_scale')
                        fail_list.append('fail') 
                else:
            ...
            count2 = uut.execute("show nve vni | excl vxlan | count")
            if int(count2) < vlan_vni_scale:
                log.info('vni count < vlan_vni_scale in CLI')
                fail_list.append('fail') 
...
class TC00001_vrf_l3vni_count_check_icam(aetest.Testcase):
    ###    This is de  scription for my testcase two
    ...
    @aetest.test
    def vrf_count_check(self):
        """ testcase clean up """
        fail_list = []
        for uut in site1_bgw_uut_list:
            op = uut.execute('show icam scale vxlan | inc VRF')
            for line in op.splitlines():
                if 'VRF' in line:
                    count = int(line.split()[4])
                    if count == routing_vlan_scale:
                        log.info('vrf count equel to routing_vlan_scale')     
                    elif count > routing_vlan_scale:
                        log.info('vrf count > routing_vlan_scale')    
                    elif count < routing_vlan_scale:
                        log.info('vrf count < routing_vlan_scale')
                        fail_list.append('fail')
                else:
            ...
            count2 = uut.execute("show nve vni | inc vxlan | count")
            if int(count2) < routing_vlan_scale:
                log.info('vrf count < routing_vlan_scale in CLI')
                fail_list.append('fail') 
...
class TC00011_vxlan_trm_NON_TRM_traffic_enable(aetest.Testcase):
    ###    This is description for my testcase two
    ...
    def setup(self):
        ...
        pcall(saveConfigtoBootflash1,uut=tuple(uut_list),filename=tuple(filename_list))
   
        log.info(banner("Vxlan Msite L2/L3/UC/BUM/V6/ traffic"))

        vxlantrafficSetupfullScaled(site1_bgw_uut_list[0],port_handle_sw1site1,\
            port_handle_sw1site2,vlan_start,vlan_vni_scale,rate,test_l3_vni_scale)
    ...
    def traffic_test(self, testscript, testbed):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_non_trm_only):
            if not rateTest(port_hdl,exp_rate):
                log.info('rateTest failed for %r',port_hdl)
                self.failed()
...
class TC00011_vxlan_trm_mcast_traffic_enable(aetest.Testcase):
    ...
    def setup(self):
        ...
        global site1_group1,site2_group1,site3_group1,group_list
        log.info(banner("saveConfigBootflash"))
        ...
        log.info(banner("----Generating traffic ----"))
 
        for i in range(0,group_scale_per_port):
            msiteTrmTrafficTestConfigure(site2_bgw_uut_list[0],port_handle_sw1site1,[port_handle_sw1site2,port_handle_sw1site3],'1000',\
                site1_group1,trm_test_l3_vlan_scale)
            msiteTrmTrafficTestConfigure(site2_bgw_uut_list[0],port_handle_sw1site2,[port_handle_sw1site1,port_handle_sw1site3],'1000',\
                site2_group1,trm_test_l3_vlan_scale)
            msiteTrmTrafficTestConfigure(site2_bgw_uut_list[0],port_handle_sw1site3,[port_handle_sw1site1,port_handle_sw1site2],'1000',\
                site3_group1,trm_test_l3_vlan_scale)
        ...
        log.info(banner("External Mcst traffic"))

        #extTrmTraffic(site1_bgw_uut_list[0],port_handle_ext1site1,[port_handle_sw1site1,\
        #    port_handle_sw1site2,port_handle_sw1site3],'239.111.111.111','144.1.1.100',vlan1='1001')
    ...
    @aetest.test
    def traffic_test(self, testscript, testbed):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm_only):
            if not rateTest(port_hdl,exp_rate):
                log.info('rateTest failed for %r',port_hdl)
                self.failed()
...
class TC00000333_vxlan_ms_add_rem_dup_ip(aetest.Testcase):
    ...
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
            if not rateTest(port_hdl,exp_rate):
                log.info('rateTest failed for %r',port_hdl)
                if not setupTrafficTest1(trm_port_handle_list,rate_list_trm,bgw_uut_list):
                    self.failed(goto=['common_cleanup'])
                
    @aetest.test
    def dupe_ip_add_remove(self, testscript, testbed):

        loAd01 = site1_bgw_uut_list[0].execute('show running-config interface loopback100 | be loopback100')
        site1_leaf_uut_list[0].configure(loAd01)
        countdown(180)
        site1_leaf_uut_list[0].configure('no interface loopback100')
...
class TC000001_vxlan_ms_snooping_disable_enable(aetest.Testcase):
    ###    This is description for my tecase two
    ...
    @aetest.test
    def snoopingdisableEnable(self, testscript, testbed):
        log.info(banner("Starting Trigger11ClearArpMac @ bgw"))

        for uut in site1_bgw_uut_list:
            for i in range(1,5):
                uut.configure("no ip igmp snooping vxlan")
                uut.configure("ip igmp snooping vxlan")
...
class TC00000333_vxlan_ms_rem_add_repl_opt(aetest.Testcase):
    ###    This is description for my tecase two
    ...
    @aetest.test
    def RemAddReplicationOptimized(self, testscript, testbed):
        log.info(banner("Starting Trigger11ClearArpMac @ bgw"))
        if not pcall(NveRemAddReplicationOptimized,uut=tuple(site1_bgw_uut_list)):
            self.failed()
...
class TC00021_vxlan_ms_trm_L2_vlan_suspend_active(aetest.Testcase):
    ###    This is description for my tecase two
    ...
    @aetest.test
    def Trigger15VlansuspendActive(self, testscript, testbed):
        log.info(banner("Starting Trigger15VlanShutNoshut bgw"))
        cmd_check = 'show vlan id 1001'
        for uut in site1_bgw_uut_list:
            vlansuspent = \
            """
            vlan {vlan1}-{vlan2}
            ...
            exit
            """
            uut.configure(vlansuspent.format(vlan1=str(vlan_start),vlan2=str(vlan_start+vlan_vni_scale)))
            
            countdown(6)
           
            if not 'suspended' in uut.execute(cmd_check):
                log.info('vlan no suspend')
                self.failed()
        ...
        for uut in site1_bgw_uut_list:
            ...
            exit
            """
            uut.configure(vlanactive.format(vlan1=str(vlan_start),vlan2=str(vlan_start+vlan_vni_scale)))

            countdown(20)

            if not 'active' in uut.execute(cmd_check):
                log.info('vlan not active')
                self.failed()
...
class TC000522_vxlan_ms_trm_restart_ngmvpn(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def ngmvpnrestart(self, testscript, testbed):
        log.info(banner("Starting restart ngmvpn @ 8"))
        for uut in site1_bgw_uut_list:
...
class TC00051_vxlan_ms_trm_restart_igp(aetest.Testcase): 
   ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
    ...
    @aetest.test
    def igprestart(self, testscript, testbed):
        log.info(banner("Starting restart igp  @ 8"))
        for uut in site1_bgw_uut_list:
            uut.execute('restart {igp} UNDERLAY'.format(igp=igp))
        countdown(20)
...
class TC00052_vxlan_ms_trm_restart_bgp(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def bgprestart(self, testscript, testbed):
        log.info(banner("Starting restart BGP  @ 8"))
        for uut in site1_bgw_uut_list:
...
class TC00053_vxlan_ms_trm_restart_pim(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def pimrestart(self, testscript, testbed):
        log.info(banner("Starting restart PIM @ 8"))
        for uut in site1_bgw_uut_list:
...
class TC00054_vxlan_ms_trm_restart_igmp(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def igmprestart(self, testscript, testbed):
        log.info(banner("Starting restart IGMP @ 8"))
        for uut in site1_bgw_uut_list:
...
class TC00055_vxlan_ms_trm_restart_mld(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def mldrestart(self, testscript, testbed):
        log.info(banner("Starting restart MLD @ 8"))
        for uut in site1_bgw_uut_list:
...
class TC00014_vxlan_trm_restart_l2rib(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def l2ribrestart(self, testscript, testbed):
        ...
        #for uut in site1_bgw_uut_list:
            #uut.execute('show tech nve >> bootflash:nve-tech-pre-l2rib',timeout=300)
            #uut.execute('show tech vxlan-evpn  >> bootflash:evpn-tech-pre-lrrib',timeout=300)
...
class TC00015_vxlan_trm_restart_l2fm(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def l2fmrestart(self, testscript, testbed):
        ...
        #for uut in site1_bgw_uut_list:
            #uut.execute('show tech nve >> bootflash:nve-tech-pre-l2fm',timeout=300)
            #uut.execute('show tech vxlan-evpn  >> bootflash:evpn-tech-pre-l2fm',timeout=300)
...
class TC00016_vxlan_trm_restart_nve(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def nverestart(self, testscript, testbed):
        ...
        #for uut in site1_bgw_uut_list:
            #uut.execute('show tech nve >> bootflash:nve-tech-pre-nve',timeout=300)
            #uut.execute('show tech vxlan-evpn  >> bootflash:evpn-tech-pre-nve',timeout=300)
...
class TC00078_vxlan_ms_trm_vlan_mgr_restart(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def Vlan_mgr_restart(self, testscript, testbed):
        ...
        log.info(banner("Starting restart VLAN MGR @ 8"))
        for uut in site1_bgw_uut_list:
            for i in range(1,2):
...
class TC00079_vxlan_ms_trm_ethpm_restart(aetest.Testcase): 
   ###    This is description for my tecase two
    ...
    def ethpmrestart(self, testscript, testbed):
        log.info(banner("Starting restart ethpm @ 8"))
        for uut in site1_bgw_uut_list:
            for i in range(1,2):
...
class TC00080_vxlan_ms_trm_igp_restart2(aetest.Testcase): 
   ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
    ...
    @aetest.test
    def igprestart1(self, testscript, testbed):
        log.info(banner("Starting restart IGP @ 8"))
        for uut in site1_bgw_uut_list:
            for i in range(1,2):
...
class TC00029_vxlan_ms_trm_fabric_link_failover1(aetest.Testcase):
   ###    This is description for my tecase two
    ...
    def bgwfabriclinkfailover1(self, testscript, testbed):
        log.info(banner("Starting bgwfabriclinkfailover1 @ 8"))
        for uut in [site1_bgw_uut_list[0]]:
            for intf in [*uut.interfaces.keys()]:
...
class TC00042_1_vxlan_ms_trm_dci_link_flap(aetest.Testcase):
   ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
    ...
    @aetest.test
    def dci1linkflap(self, testscript, testbed):       
        for uut in [site1_bgw_uut_list[1]]:
            for intf in [*uut.interfaces.keys()]:
                if 'site1_dci2' in intf:
                    intf = uut.interfaces[intf].intf
...
class TC00043_vxlan_ms_trm_igp_shut_noshut(aetest.Testcase):
   ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
    ...
    @aetest.test
    def remotebgwigpshut(self, testscript, testbed):
        log.info(banner("Starting TriggerCoreIfFlap222 @ 8"))
        ...
        site1_bgw_uut_list[0].configure(cfg_shut.format(igp=igp))
        site1_bgw_uut_list[1].configure(cfg_shut.format(igp=igp))
        countdown(15)
        site1_bgw_uut_list[0].configure(cfg_noshut.format(igp=igp))
        site1_bgw_uut_list[1].configure(cfg_noshut.format(igp=igp))
        countdown(20)
...
class TC00045_vxlan_ms_trm_spine1_igp_shut(aetest.Testcase):
   ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
    ...
    @aetest.test
    def remotebgwigpshut(self, testscript, testbed):
        log.info(banner("Starting anycast_bgw_spine1_igp_shut @ 8"))
        cfg_shut =\
        """
        router {igp} UNDERLAY
        shut
        ...
        site1_spine_uut_list[0].configure(cfg_shut.format(igp=igp))
        countdown(20)
    ...
    @aetest.test
    def remotebgwigpnoshut(self, testscript, testbed):
        ...
        site1_spine_uut_list[0].configure(cfg_noshut.format(igp=igp))
        countdown(20)
...
class TC00046_vxlan_ms_trm_spine2_igp_shut(aetest.Testcase):
   ###    This is description for my tecase two
    @aetest.setup
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
    ...
    @aetest.test
    def remotebgwigpshut1(self, testscript, testbed):
        ...
        site1_spine_uut_list[1].configure(cfg_shut.format(igp=igp))
        countdown(20)
    ...
    @aetest.test
    def remotebgwigp3Noshut(self, testscript, testbed):
        ...
        no shut
        """
        site1_spine_uut_list[1].configure(cfg_noshut.format(igp=igp))
        countdown(20)
...
class TC000571_vxlan_ms_trm_local_bgw_bgp_shut(aetest.Testcase):
    ...
    def bgwbgpNoshut(self, testscript, testbed):
        log.info(banner("Starting bgwbgpNoshut @ 8"))
...
class TC00058_vxlan_ms_trm_Core_flap(aetest.Testcase):
    ...
    def Trigger5CoreIfFlap(self, testscript, testbed):
        log.info(banner("Starting L3InterfaceFlap @ 8"))
...
class TC00061_vxlan_ms_trm_clear_igp_Neigh(aetest.Testcase):
    ###    This is description for my tecase two
    ...
    @aetest.test
    def Trigger8ClearIgpNeigh(self, testscript, testbed):
...
class TC00067_vxlan_ms_trm_Spine_Clear_igp(aetest.Testcase):
    ###    This is description for my tecase two
...
class TC0001111_vxlan_ms_trm_bgw1_reload(aetest.Testcase):
    ...
    def bgwReload(self, testscript, testbed):
        ...
        try:
            ...
            if not nodeStateCheck(site1_bgw_uut_list[0],igp):
        ...
        try:
            ...
            if not nodeStateCheck(site2_bgw_uut_list[0],igp):
                log.info('nodeStateCheck Failed')
        except:
            log.info('site1_bgw_uut_list[0] reload failed ')
            self.failed()
...
class TC0001111_vxlan_ms_trm_spine_reload(aetest.Testcase):
    ...
    def bgwReload(self, testscript, testbed):
        ...
        try:
            ...
            if not nodeStateCheck(site1_spine_uut_list[0],igp):
                log.info('nodeStateCheck Failed') 
        except:
...
class TC0001111_vxlan_ms_trm_bgw2_ascii_reload(aetest.Testcase):
    ...
    def bgwReload(self, testscript, testbed):
        ...
        try:
            ...
            if not nodeStateCheck(site1_bgw_uut_list[1],igp):
                log.info('nodeStateCheck Failed')
...
class TC0001920_vxlan_ms_trm_Configure_replace_bg_gwy(aetest.Testcase):
    ###    This is description for my tecase two
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
            if not rateTest(port_hdl,exp_rate):
    ...
    @aetest.test
    def mstrmConfigReplacebgw(self, testscript, testbed):
        ...
        for uut in [site1_bgw_uut_list[0]]:
            ...
            countdown(2)
            uut.configure("no evpn multisite border-gateway 65001 ")
            countdown(2)                 
...
class TC000192_vxlan_ms_trm_Configure_replace_nv_overlay(aetest.Testcase):
    ...
    def mstrmConfigReplace(self, testscript, testbed):
        ...
        #for uut in site1_bgw_uut_list:
            #uut.execute('show tech nve >> bootflash:nve-tech-pre-cr-nve',timeout=300)
            #uut.execute('show tech vxlan-evpn  >> bootflash:evpn-tech-pre-cr-nve',timeout=300)
...
class TC000195_vxlan_ms_trm_rollback_after_bgp_remove(aetest.Testcase):
    ###    This is description for my tecase two
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
            if not rateTest(port_hdl,exp_rate):
    ...
    @aetest.test
    def mstrmrollbackbgpremove(self, testscript, testbed):
        
        result_list = []
        
        for uut in [site1_bgw_uut_list[0]]:
            tm = uut.configure("show clock | excl Time")
            log.info("time is ----- %r",tm)
            #tm1 =  tm.replace(":","").replace(".","").replace(" ","")

            name = str(randint(1000,2000))

            name = 'mstrm_'+name
            name_bk = 'bk_mstrm_'+name
            uut.configure('copy run bootflash:{name_bk}'.format(name_bk=name_bk))
            uut.configure('checkpoint {name}'.format(name=name))
            countdown(2)
            uut.configure("no feature bgp")
            countdown(2)                 
            op = uut.configure('rollback running-config checkpoint {name} stop-at-first-failure'.format(name=name),timeout=120)
            if not "successfully" in op:
                uut.execute('show rollback status ')
                uut.execute('show rollback log exec')
                uut.execute('show rollback log verify')
                self.failed(goto=['common_cleanup'])
...
class TC000195_vxlan_ms_trm_rollback_after_nve_remove(aetest.Testcase):
    ###    This is description for my tecase two
    def setup(self):
        for port_hdl,exp_rate in zip(trm_port_handle_list,rate_list_trm):
            if not rateTest(port_hdl,exp_rate):
    ...
    @aetest.test
    def mstrmrollbacknveremove(self, testscript, testbed):
        
        result_list = []
        
        for uut in [site2_bgw_uut_list[0]]:
            name = str(randint(2001,3000))
            name = 'mstrm_'+name
            name_bk = 'bk_mstrm_'+name
            uut.configure('copy run bootflash:{name_bk}'.format(name_bk=name_bk))
            uut.configure('checkpoint {name}'.format(name=name))
