...
def configVmctDomain(vmct_config_dict,hdl,log):
      ...
      log.info(banner(f'Config VMCT Domain on {hdl}'))
      ...
      cfg+='''peer-keepalive destination {0} source {1} vrf {2} 
           ...
           '''.format(ns_domain.peer_keepalive_src_ipv4_addr,ns_domain.peer_keepalive_dst_ipv4_addr,ns_domain.peer_keepalive_vrf,ns_domain.virtual_peer_dst_addr,ns_domain.virtual_peer_src_addr)
      out=hdl.configure(cfg,timeout=150)
      if re.search('error|invalid',out,re.I):
          log.error(f'Configuring VMCT Domain failed on {hdl}')
          return 0
      return 1

def configVmctPeerLink(vmct_config_dict,hdl,log):
      log.info(banner(f'Config VMCT PeerLink on {hdl}'))
      ...
      out=hdl.configure(cfg,timeout=150)
      if re.search('error|invalid',out,re.I):
          log.error(f'Configuring VMCT Peer link failed on {hdl}')
          return 0
      return 1


def configVmctPO(vmct_config_dict,hdl,log):
      log.info(banner(f'Config VMCT PO on {hdl}'))
      cfg=''
      ...
      out=hdl.configure(cfg,timeout=150)
      if re.search('error|invalid',out,re.I):
            log.error(f'Configuring VMCT PO failed on {hdl}')
            return 0
      return 1 
     
def configVmctFabricLinks(vmct_config_dict,hdl,log):
        log.info(banner(f'Config VMCT FabricLinks on {hdl}'))
        cfg=''
        ...
        out=hdl.configure(cfg,timeout=150)
        if re.search('error|invalid',out,re.I):
            log.error(f'Configuring VMCT Fabric link config failed on {hdl}')
            return 0
        return 1 
...
def configVmctCoreFabricLinks(vmct_config_dict,hdl,log):
         ...
         log.info(banner(f'Config VMCT Core FabricLinks on {hdl}'))
         cfg=''
         ...
         out=hdl.configure(cfg,timeout=150)
         if re.search('error|invalid',out,re.I):
            log.error(f'Configuring VMCT Core Fabric link config failed on {hdl}')
            return 0
         return 1 
