                   ...
                   if re.search('uut[\d]+_[uutTG]+[\d]+_[\d]+.\d+',str(key)):
                     ...
                     conf.pop(keys,None)
                      ...
                      if re.search('(uut+[\d]+)_[uutTG]+[\d]+_[\d]+', str(key)):
                         ...
                         keylist = re.search('(TG[\d]+)_[uut]+[\d]+_[\d]+', str(key))
...
class configSetup():
    ...
    def invoke(config_dict,testbedObj,logger):
        ...
        for dut in testbedObj.devices.aliases:
            for node in testbedObj.devices:
                if testbedObj.devices[node].alias == dut:
                    if testbedObj.devices[node].type != 'ixia':
                        dut_list.append(dut)
        #     if dut != 'tgen':
        #         dut_list.append(dut)
        ...
        #     hdl.connect(cls=Unicon,via='vty')
            hdl.connect()
            hdl_list.append(hdl)
