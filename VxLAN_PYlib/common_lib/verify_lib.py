...
class verifyShowProcessLogs():
  ...
  def __init__(self, hdl, log):
      ...
      self.result='pass'
      show_process_log=hdl.execute('show process log')
      ## 
      ...
      for key, cmd in cmd_dict.items():
          if key in cc_params or 'all' in cc_params:
             if key == 'ipv4' or key == 'ipv6':
                op = hdl.execute('test consistency-checker forwarding {0} vrf all'.format(key))
                ...
                op = hdl.execute('test forwarding {0} inconsistency module 1'.format(key1))
                if not re.search('Consistency check started.',op):
                   testResult('fail','Consistency checker not started for {0}'.format(key1),log)
                   continue
                time.sleep(5)
                result += checkCCResult(hdl, cmd, log)
                #hdl.execute('test forwarding {0} inconsistency module 1 stop'.format(key1))
...
class verifyShowCores():
  ...
  def __init__(self, hdl, log,*args):
      ...
      hdl.execute('show logging logfile | grep -i crash')
      show_cores=hdl.execute('show cores')
...
class verifyModules(object):
    ...
    def __init__(self,hdl, log, *args):
        ...
        if not result:
            hdl.execute('delete bootflash:/sh_tech_mod_all no-prompt')
            hdl.execute('show tech-support module all > bootflash:/sh_tech_mod_all',timeout=180)
...
class verifySyslogs():
  ...
  def __init__(self,hdl,log,syslogcheckdict,startlog, *args):
    ...
    mandatory_list={}
    if startlog:
        logoutput=hdl.execute("show logging last 9999 | begin \"{0}\"".format(startlog))
    else:
        logoutput=hdl.execute("show logging last 9999")
...
def verifyNoSpanningTreeForL3Port(hlite,log,hdl,interface):
        ...
        show_interface_log=hdl.execute('show spanning-tree interface {0}'.format(interface))
