...
class ixiaPyats_lib:
    ...
    def connect_to_ixia (args_dict):
        ...
        if 'reset' not in args_dict.keys():
                args_dict['reset'] = 1
        if 'tcl_port' not in args_dict.keys():
                args_dict['tcl_port'] = '8009'
        
        ixTclServer = str(args_dict['tcl_server']) + ":" + str(args_dict['tcl_port'])
        
        if 'session_resume_keys' in args_dict.keys():
            if args_dict['session_resume_keys'] == 0:
                _result_ = ixiangpf.connect(
                    ixnetwork_tcl_server    = ixTclServer,
                    session_resume_keys     = args_dict['session_resume_keys'],
                    connect_timeout         = 180,
                )
        else:
            _result_ = ixiangpf.connect(
        ...
        if _result_['status'] == '1':
            print("Ixia connection successful")
            log.info("Ixia connection successful")
            return _result_
        else:
            print("Ixia connection Failed" + str(_result_['log']))
    ...
    ###################################################################
    ###           Interface Configuration Libraries                 ###
    ###################################################################
    ...
    ###################################################################
    ###           Traffic Item Configuration Libraries              ###
    ###################################################################

    #====================================================================================================#
    @staticmethod
    def delete_traffic_item(args_dict):
        log.info("--> In delete_traffic_item")
        #Define arguments definition
        args_def = [
                ('mode'              , 'o', [str]),
                ('stream_id'         , 'm', [str]),
        ...
        _result_ = ixiangpf.traffic_config(traffic_generator = 'ixnetwork_540', mode = 'remove', stream_id = args_dict['stream_id'])
        log.info("_result_['status']:"+str(_result_['status']))
        log.info(_result_)

        if _result_['status'] == "1":
            log.info("Deleted Traffic Item successfully")
            return 1
        else:
            log.info("Deleting Traffic Item Failed")
            return 0
        
    #====================================================================================================#
    @staticmethod
    def edit_traffic_item(args_dict):
        ...
        _result_ = ixiangpf.traffic_config(traffic_generator = 'ixnetwork_540', mode = 'modify', stream_id = args_dict['stream_id'], rate_pps = args_dict['rate_pps'],)
        log.info("_result_['status']:"+str(_result_['status']))
        log.info(_result_)

        if _result_['status'] == "1":
            log.info("Edited Traffic Item successfully")
            return 1
        else:
            log.info("Editing Traffic Item Failed")
            return 0
    ...
    #====================================================================================================#
    @staticmethod
    def configure_ixia_traffic_item_v46(args_dict):
        ...
        args_def = [
                ...
                ('bi_dir'       , 'm', [int, bool]),
                ('ip_dscp'      , 'o', [int]),
                ('vlan_user_priority'      , 'o', [str]),
                ('data_tos'     , 'o', [int]),
                ('ip_precedence' , 'o', [int]),
                ('ipv6_traffic_class' , 'o', [int]),
                ('enable_data_integrity' , 'o', [int]),
                ('bidirectional'  ,  'o', [str]),
                ]
        ...
        try:
            ...
            #log.info(help_string)
            return 0
        if 'ip_dscp' not in args_dict.keys():
            args_dict['ip_dscp'] = 0
        if 'ip_precedence' not in args_dict.keys():
            args_dict['ip_precedence'] = 0
        if 'ipv6_traffic_class' not in args_dict.keys():
            args_dict['ipv6_traffic_class'] = 0
        ...
        _result_ = ixiahlt.traffic_config(
            ...
            length_mode                 = 'auto',
            rate_pps                    = args_dict['rate_pps'],
#            data_tos                    = args_dict['data_tos'],
            ip_dscp                     = args_dict['ip_dscp'],
#            ip_precedence               = args_dict['ip_precedence'],
#            ipv6_traffic_class          = args_dict['ipv6_traffic_class'],
#            vlan_user_priority          = args_dict['vlan_user_priority'],
            #enable_data_integrity       = args_dict['enable_data_integrity'],
    ...
    def configure_ixia_traffic_item(args_dict):
        ...
        args_def = [
                ...
                ('rate_pps'                 , 'm', [str]),
                ('bi_dir'                   , 'm', [str, int, bool]),
                ('end_point_set'            , 'o', [str, int, bool]),
                ('scalable_dsts_port_count' , 'o', [str, int, bool]),
                ('scalable_dsts_intf_count' , 'o', [str, int, bool]),
                ('ip_dscp'                  , 'o', [int]),
                ('vlan_user_priority'       , 'o', [str]),
                ('data_tos'                 , 'o', [int]),
                ('ip_precedence'            , 'o', [int]),
        ...
        if 'ipv6_traffic_class' not in args_dict.keys():
            args_dict['ipv6_traffic_class'] = 0
        if 'vlan_user_priority' not in args_dict.keys():
            args_dict['vlan_user_priority'] = 0

        if 'scalable_dsts_port_count' in args_dict.keys() or 'scalable_dsts_intf_count' in args_dict.keys():

            ti_dsts = {'EndpointSet-1': ''}
            scalable_dsts_port_count    = 1
            scalable_dsts_intf_count    = 1

            if 'scalable_dsts_port_count' in args_dict.keys():
                scalable_dsts_port_count = args_dict['scalable_dsts_port_count']
                log.info('scalable_dsts_port_count - ' + str(scalable_dsts_port_count))
                log.info('scalable_dsts_port_count - ' + str(scalable_dsts_port_count))
            if 'scalable_dsts_intf_count' in args_dict.keys():
                scalable_dsts_intf_count = args_dict['scalable_dsts_intf_count']
                log.info('scalable_dsts_intf_count - ' + str(scalable_dsts_intf_count))
                log.info('scalable_dsts_intf_count - ' + str(scalable_dsts_intf_count))

            ixiatcl.set_py('ti_scalable_dsts(EndpointSet-1)', args_dict['dst_hndl'])
            ixiatcl.set_py('ti_scalable_dsts_port_start(EndpointSet-1)', [1])
            ixiatcl.set_py('ti_scalable_dsts_port_count(EndpointSet-1)', [scalable_dsts_port_count])
            ixiatcl.set_py('ti_scalable_dsts_intf_start(EndpointSet-1)', [1])
            ixiatcl.set_py('ti_scalable_dsts_intf_count(EndpointSet-1)', [scalable_dsts_intf_count])

            log.info("Received scalable dst handles - new handle is ->")
            log.info(ti_dsts['EndpointSet-1'])
            log.info("Received scalable dst handles - new handle is ->")
            log.info(ti_dsts['EndpointSet-1'])
    ...
    #====================================================================================================#
    @staticmethod
    def configure_ixia_traffic_item_tos(args_dict):
    ...
    #====================================================================================================#
    @staticmethod
    def configure_ixia_traffic_item_cos(args_dict):
        ...
        args_def = [
                ...
                ('ipv6_traffic_class'       , 'o', [int]),
                ('enable_data_integrity'    , 'o', [int]),
                ('egress_tracking'          , 'o', [str]),
                ('egress_tracking_encap'    , 'o', [str]),
                ('track_by'                 , 'o', [str]),
                ('bidirectional'            , 'm', [str]),
            ]
        ...
        _result_ = ixiahlt.traffic_config(
            ...
            ip_dscp                     = args_dict['ip_dscp'],
            vlan_user_priority          = args_dict['vlan_user_priority'],
            track_by                    = args_dict['track_by'],
            egress_tracking_encap       = args_dict['egress_tracking_encap'],
            egress_tracking             = args_dict['egress_tracking'],
            # data_tos                  = args_dict['data_tos'],
            # ip_precedence             = args_dict['ip_precedence'],
            # ipv6_traffic_class        = args_dict['ipv6_traffic_class'],
            # enable_data_integrity     = args_dict['enable_data_integrity'],
    ...
    #====================================================================================================#
    @staticmethod
    def configure_ixia_IP_UCAST_COS_traffic_item(args_dict):
        ...
        _result_ = ixiahlt.traffic_config(
                            ...
                            ip_src_step                    = args_dict['ip_src_step'],
                            ip_dst_addr                    = "255.255.255.255",
                            ip_dst_mode                    = "increment",
                            ip_dst_count                   = args_dict['vlanid_count'],
                            ip_dst_step                    = args_dict['ip_dst_step'],
                            track_by                       ='sourceDestEndpointPair0 trackingenabled0',
                            )
        log.info("_result_['status']:"+str(_result_['status']))
        log.info(_result_)
    ...
    #====================================================================================================#
    @staticmethod
    def configure_ixia_traffic_item_ipv6_cos(args_dict):
    ...
    #====================================================================================================#
    @staticmethod
    def configure_ixia_l3_traffic_item(args_dict):
        ...
        _result_ = ixiahlt.traffic_config(
                            ...
                            ip_src_count                    = args_dict['vlanid_count'],
                            ip_src_step                     = args_dict['ip_src_step'],
                            ip_dst_addr                     = args_dict['ip_dst_addrs'],
                            ip_dst_mode                     = "increment",
                            ip_dst_count                    = args_dict['vlanid_count'],
                            ip_dst_step                     = args_dict['ip_dst_step'],
                            track_by                       ='sourceDestEndpointPair0 trackingenabled0',
...
#====================================================================================================#
    @staticmethod
    def configure_ixia_l3_traffic_item_ipv6(args_dict):
        ...
        args_def = [
            ...
            ('circuit'          , 'm', [str]),
            ('vlan_user_priority', 'm', [str]),
            ('ipv6_src_addrs'     , 'm', [str]),
#            ('ipv6_step'          , 'm', [str]),
            ('ipv6_dst_addrs'     , 'm', [str]),
#            ('ip_src_count'     , 'm', [str]),
            ('ipv6_src_step'      , 'm', [str]),
            ('ipv6_dst_step'      , 'm', [str]),
        ]
        ...
        _result_ = ixiahlt.traffic_config(
                            ...
                            vlan_user_priority             = args_dict['vlan_user_priority'],
                            #ip_dscp                        = args_dict['ip_dscp'],
                            ipv6_src_addr                     = args_dict['ipv6_src_addrs'],
                            ipv6_src_mode                     = "increment",
                            ipv6_src_count                    = args_dict['vlanid_count'],
                            ipv6_src_step                     = args_dict['ipv6_src_step'],
                            ipv6_dst_addr                     = args_dict['ipv6_dst_addrs'],
                            ipv6_dst_mode                     = "increment",
                            ipv6_dst_count                    = args_dict['vlanid_count'],
                            ipv6_dst_step                     = args_dict['ipv6_dst_step'],
                            track_by                       ='sourceDestEndpointPair0 trackingenabled0',
                            )
        log.info("_result_['status']:"+str(_result_['status']))
        log.info(_result_)
    ...
    # ====================================================================================================#
    @staticmethod
    def configure_ixia_L2_UCAST_traffic_item(args_dict):
    ...
    # ====================================================================================================#
    @staticmethod
    def configure_ixia_raw_vlan_traffic(args_dict):
    ...
    # ====================================================================================================#
    @staticmethod
    def configure_v4_mcast_traffic_item_per_tag_QoSValidation(args_dict):
    ...
    #====================================================================================================#
    @staticmethod
    def start_traffic(traffic_item_list=[]):
        
        if traffic_item_list == []:
            _result = ixiangpf.traffic_control(action='run')
        else:
            _result = ixiangpf.traffic_control(action='run',handle=traffic_item_list)
        if _result['status'] == '1':
    ...
    ###################################################################
    ###            Traffic Stats Validation Libraries               ###
    ###################################################################

    #====================================================================================================#
    def get_traffic_item_names(self):
        
        traffic_items = []
        
        #--------------------------------------------------------------------------------#
        # Apply Traffic to get stats ready
        log.info("---- Applying Traffic --- ")
        action_result = "Passed: Applying Traffic Successfully" if self.apply_traffic() == 1 else "Failed: To Apply Traffic Item"
        log.info(action_result)
        if 'Fail' in action_result: return 0
        ...
        for traffic_stats_retry in range(5):
            r = ixiangpf.traffic_stats(mode = 'traffic_item',)
        ...
        for item in r['traffic_item']:
            if re.match('TI\\d+', item, re.I):
                traffic_items.append(str(item))

        if traffic_items != []:
            return traffic_items.sort()
        else:
            return 0

    #====================================================================================================#
    def populate_traffic(self,waitTimeMultiplier=1,traffic_item_list=[]):

        #--------------------------------------------------------------------------------#
        # Populating stats
        log.info("---- Clearing Statistics --- ")
        action_result = "Passed: Stats cleared successfully" if self.clear_traffic_stats() == 1 else "Failed: To clear Stats"
        log.info(action_result)
        if 'Fail' in action_result: return 0
        ...
        if traffic_item_list != []:
            action_result = "Traffic Started successfully" if self.start_traffic(traffic_item_list) == 1 else "Failed: To Start traffic"
        else:
            action_result = "Traffic Started successfully" if self.start_traffic() == 1 else "Failed: To Start traffic"
        log.info(action_result)
        if 'Fail' in action_result: return 0
        ...
        action_result = "Traffic Stopped successfully" if self.stop_traffic() == 1 else "Failed: To Stop traffic"
        log.info(action_result)
        if 'Fail' in action_result: return 0
    ...
    #====================================================================================================#
    # def verify_traffic(self,threshold=2,waitTimeMultiplier = 1, return_individual_TI = 0):
    def verify_traffic(self,*args):
        
        #--------------------------------------------------------------------------------#
        # Validate parameters
        # Define Arguments Definition
        args_def = [
                ('loss_threshold'               , 'o', [str, int]),
                ('waitTimeMultiplier'           , 'o', [str, int]),
                ('return_individual_TI'         , 'o', [str, int]),
                ('traffic_item_list'            , 'o', [list]),
                ]
        ...
        if type(args) == tuple:
            ...
            # Backward compaitability - direct args passing
            loss_percentage_threshold = args[0] if len(args) >= 1 and type(args[0]) != dict else 2
            waitTimeMultiplier        = args[1] if len(args) >= 2 and type(args[0]) != dict else 1
            return_individual_TI      = args[2] if len(args) == 3 and type(args[0]) != dict else 0
            
            # new Implementation with validation arguments - will overwrite the old implemented values
            if len(args) >= 1:
                if type(args[0]) == dict:
                    try:
                        ArgVal.validate(args_def, **args[0])
                    except Exception as e:
                    ...
                    loss_percentage_threshold = args[0]['loss_threshold'] if 'loss_threshold' in args[0].keys() else 2
                    waitTimeMultiplier        = args[0]['waitTimeMultiplier'] if 'waitTimeMultiplier' in args[0].keys() else 1
                    return_individual_TI      = args[0]['return_individual_TI'] if 'return_individual_TI' in args[0].keys() else 1
                    traffic_item_list         = args[0]['traffic_item_list'] if 'traffic_item_list' in args[0].keys() else []
        
        # Print the parameter values
        print('loss_percentage_threshold - '+str(loss_percentage_threshold))
        print('waitTimeMultiplier - '+str(waitTimeMultiplier))
        print('return_individual_TI - '+str(return_individual_TI))
        print('traffic_item_list - '+str(traffic_item_list))
        #--------------------------------------------------------------------------------#
        # Declare variables
        fail_flag = 0
        traffic_item_dict = {}
        TrafficItemTable = texttable.Texttable()
        TrafficItemTable.header(['Traffic Item', 'Loss % Observed\nThreshold - '+str(loss_percentage_threshold)+' %', 'Status','Remarks'])
        TrafficItemTable.set_cols_width([40,20,20,50])
        
        #--------------------------------------------------------------------------------#
        # Populating stats
        if self.populate_traffic(waitTimeMultiplier,traffic_item_list) == 0:
            log.info("Could not populate traffic stats (clear, start, stop)")
            print("Could not populate traffic stats (clear, start, stop)")
            return 0
        ...
        if traffic_item_list == []:
            for item in r['traffic_item']:
                if re.match('TI\\d+', item, re.I):
                    traffic_item_dict[str(item)] = {}
                    traffic_item_dict[str(item)]['loss_percentage'] = str(r['traffic_item'][item]['rx']['loss_percent'])
                    traffic_item_dict[str(item)]['tx_pkts'] = str(r['traffic_item'][item]['tx']['total_pkts'])
                    traffic_item_dict[str(item)]['rx_pkts'] = str(r['traffic_item'][item]['rx']['total_pkts'])
        else:
            for item in r['traffic_item']:
                if (re.match('TI\\d+', item, re.I)) and (item in traffic_item_list):
                    traffic_item_dict[str(item)] = {}
                    traffic_item_dict[str(item)]['loss_percentage'] = str(r['traffic_item'][item]['rx']['loss_percent'])
                    traffic_item_dict[str(item)]['tx_pkts'] = str(r['traffic_item'][item]['tx']['total_pkts'])
                    traffic_item_dict[str(item)]['rx_pkts'] = str(r['traffic_item'][item]['rx']['total_pkts'])

        log.info("Traffic Items found:")
        log.info(traffic_item_dict)

        #--------------------------------------------------------------------------------#
        # Validating Stats
        for TI in traffic_item_dict.keys():
            
            # If the loss percentage is N/A, check the pkt count
            if traffic_item_dict[TI]['loss_percentage'] == 'N/A':
                if int(traffic_item_dict[TI]['rx_pkts']) > int(traffic_item_dict[TI]['tx_pkts']):
                    TrafficItemTable.add_row([TI, str(r['traffic_item'][item]['rx']['loss_percent']), 'Pass w/ Exception', "Rx("+str(traffic_item_dict[TI]['rx_pkts'])+") is > Tx("+str(traffic_item_dict[TI]['tx_pkts'])+")"])
                else:
                    TrafficItemTable.add_row([TI, str(r['traffic_item'][item]['rx']['loss_percent']),'FAIL', "Rx(" + str(traffic_item_dict[TI]['rx_pkts']) + ") is < Tx(" + str(traffic_item_dict[TI]['tx_pkts']) + ")"])
                    fail_flag = 1
            
            # If the loss percentage is present
            else:
                if float(traffic_item_dict[TI]['loss_percentage']) <= float(loss_percentage_threshold):
                    TrafficItemTable.add_row([TI, traffic_item_dict[TI]['loss_percentage'],'PASS',''])
                else:
                    TrafficItemTable.add_row([TI, traffic_item_dict[TI]['loss_percentage'], 'FAIL',''])
                    fail_flag = 1

        log.info(banner("Threshold for Traffic Item verification is -> "+str(loss_percentage_threshold)))
        log.info(TrafficItemTable.draw())

        #--------------------------------------------------------------------------------#
        # Return - Simple return for backward compaitability
        if return_individual_TI != 0:
            if fail_flag == 1:
                return {'status':0, 'individual_TI' : traffic_item_dict}
            else:
                return {'status':1, 'individual_TI' : traffic_item_dict}
        else:
            if fail_flag == 1:
                return 0
            else:
    ...
    # ====================================================================================================#
    def verify_traffic_live(self, threshold, waitTimeMultiplier=1, extraPktThreshold=100):

        fail_flag = 0

        TrafficItemTable = texttable.Texttable()
        TrafficItemTable.header(
            ['Traffic Item', 'Loss % Observed\nThreshold - ' + str(threshold) + ' %', 'Status', 'Remarks'])
        TrafficItemTable.set_cols_width([40, 20, 20, 50])
        ...
        if traffic_run_status != 1:
            log.info("Failed: To clear Stats")
            return 0
        else:
            log.info("\nPassed: Stats cleared successfully\n")
        ...
        for item in r['traffic_item']:
            if re.match('TI\\d+', item, re.I):
                ...
                if str(r['traffic_item'][item]['rx']['loss_percent']) == 'N/A':
                    ...
                    if rx_pkts > tx_pkts:
                        if abs(int(rx_pkts) - int(tx_pkts)) < extraPktThreshold:
                            TrafficItemTable.add_row([item, str(r['traffic_item'][item]['rx']['loss_percent']), 'Pass w/ Exception',"Rx(" + str(rx_pkts) + ") is > Tx(" + str(tx_pkts) + ") but within " + str(extraPktThreshold) + "(Threshold)"])
                        else:
                            TrafficItemTable.add_row([item, str(r['traffic_item'][item]['rx']['loss_percent']), 'FAIL',"Rx(" + str(rx_pkts) + ") is - Tx(" + str(tx_pkts) + ") is > " + str(extraPktThreshold) + "(Threshold)"])
                            fail_flag = 1
                    else:
                        TrafficItemTable.add_row([item, str(r['traffic_item'][item]['rx']['loss_percent']), 'FAIL',"Rx(" + str(rx_pkts) + ") is < Tx(" + str(tx_pkts) + ")"])
                        fail_flag = 1
                else:
                    tx_pkts_rate = int(float(r['traffic_item'][item]['tx']['total_pkt_rate']))
                    rx_pkts_rate = int(float(r['traffic_item'][item]['rx']['total_pkt_rate']))
                    ...
                    if loss_percent <= threshold:
                        ...
                        if (tx_pkts_rate == rx_pkts_rate) or (abs(int(rx_pkts_rate) - int(tx_pkts_rate)) < 10):
                            TrafficItemTable.add_row([item, loss_percent, 'PASS','Converged TxRate - ' + str(tx_pkts_rate) + ' RxRate - ' + str(rx_pkts_rate)])
                        else:
                            TrafficItemTable.add_row([item, loss_percent, 'FAIL', 'Not Converged TxRate - ' + str(
                                tx_pkts_rate) + ' RxRate - ' + str(rx_pkts_rate)])
                            fail_flag = 1
