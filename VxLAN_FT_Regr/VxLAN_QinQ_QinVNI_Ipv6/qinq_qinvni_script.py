...
class jasim_test(nxtest.Testcase):
    ...
    @aetest.test
    def checking(self, vlan_id, endpoint, testbed):
        myvlan = "{0}".format(vlan_id[0])
        log.info(myvlan)

        print (testbed.servers['radius']['credentials']['endpoint'])
        ep = testbed.servers['radius']['credentials']['endpoint']
        print (len(ep))
        print (type(ep))
        print (ep.get("host-1"))
        mylist = []
        ...
        for i in ep:
            print (i)
            opm = testbed.servers['radius']['credentials']['endpoint'][i]
            print (opm)
...
def changeInterfaceConfig(section, **kwargs):
    
    # sqinvni_dut_dict = {
    #             'node02_vpcVtep1' : {
    #                     'interface'     : ['nd02_tgen_1_1', 'po100'], 
    #                     'global_cfgs'   : 'system dot1q-tunnel transit 1001-1005',
    #                     'intf_configs'       : 'switchport ; switchport mode trunk ; \
    #                                         swithport vlan-mapping 51-55 dot1q-tunnel 1001 ; \
    #                                         swithport vlan-mapping 56-60 dot1q-tunnel 1002 ; \
    #                                         swithport vlan-mapping 61-65 dot1q-tunnel 1003 ; \
    #                                         swithport vlan-mapping 66-70 dot1q-tunnel 1004 ; \
    #                                         swithport vlan-mapping 71-75 dot1q-tunnel 1005 ; \                                        
    #                                         no shut'
    #                     },
    #             'node03_vpcVtep2' : {
