...
#     @aetest.test
# def check_egress_TC(section, testbed, testscript, **kwargs):
#     device_dut = kwargs.get('vtep_list', 1)
#     status_flag= []
#     status_msgs= '\n'
#     for node in device_dut:
...
#     if 0 in status_flag:
#         section.failed(reason=status_msgs)
#     else:
#         section.passed(reason=status_msgs)
...
def check_egress_TC(section, testbed, testscript, **kwargs):
    ...
    try:
        ...
        if 0 in status_flag:
            ...
            section.errored('Exception occurred while verifying dscp marking Leaf1', goto=['cleanup'])        

def SHUT_VPC_PO_ON_LEAF2(section, testbed, testscript, **kwargs):
    device_dut = kwargs.get('secondary_dut', 1)
    for node in device_dut: 
        try:  
          ...
          ''') 
            ...
            section.failed('Exception occurred while shutting vpc leg on LEAF-2', goto=['cleanup'])
    time.sleep(30)         

def N0_SHUT_VPC_PO_ON_LEAF2(section, testbed, testscript, **kwargs):
    device_dut = kwargs.get('secondary_dut', 1)
    for node in device_dut:  
        try:  
            testbed.devices[node].configure('''    
