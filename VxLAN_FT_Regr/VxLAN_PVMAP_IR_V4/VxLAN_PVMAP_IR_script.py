        ...
        try:
            testscript.parameters['LEAF-1'].configure('''
              ...
              interface ''' + str(testscript.parameters['intf_LEAF_1_2_to_IXIA']) + '''
                ...
                switchport access vlan ''' + str(testscript.parameters['LEAF_1_dict']['STATIC_IR_VNI_data']['l2_vlan_start']) + '''
                spanning-tree port type edge
                no shutdown
...
class TC_VXLAN_PVMAP_019(aetest.Testcase):
    ...
    def testcase_orphan(self, testscript, steps):
        ...
        with steps.start('Do Config backup on Primary, Secondary and Standalone'):
            ...
            prim_config = prim_vtep.execute("show run bgp")
            sec_config = sec_vtep.execute("show run bgp")
            sa_config = sa_vtep.execute("show run bgp")
        ...
        log.info("Waiting 60secs to remove VRF on BGP")
        time.sleep(60)

        with steps.start('Reconfigure bgp configs on vpc and standalone'):
            # try:
            #     prim_vtep.configure(prim_config)
            #     sec_vtep.configure(sec_config)
            #     sa_vtep.configure(sa_config)
            # except Exception as error:
            #     log.debug("Unable to configure - Encountered Exception " + str(error))
            #     self.errored('Exception occurred while configuring PVMAP', 
            #                 goto=['common_cleanup'])
