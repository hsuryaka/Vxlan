    ...
    def Nodes(self,node):
        ...
        if hdl.type in ['N35K']:
           ...
           for region in self.tcam_config_dict[node].keys():
                ...
                #if hdl.errFlag:
                #        self.log.error('Error while carving TCAM regions.Kindly check logs')
                #        return 0
                ...
                print('#############')
                print('reload_result')
                #if not disruptSwitch(self.log,self.hlite,'-dut {0} -action {1} -copy_flag True'.format(node,'reload')):
                #        self.result='fail'
                #        self.log.error('Reload Failure')
                #        return 0
                #logger.info(banner("Reloading of devices"))
        return 1
