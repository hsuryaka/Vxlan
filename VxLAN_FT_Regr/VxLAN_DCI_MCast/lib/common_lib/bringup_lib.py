...
class  configFeature():
    ...
    def invoke(hdl, log,*args):
        ...
        log.info('The value of feature_name is : %s' % feature_name)
        ...
        for feature in str.split(feature_name, ','):
            log.info('The value of feature inside for loop  is : %s' % feature)
            if re.search('hsrp',feature,re.I):
                sw_cmd='''feature hsrp
                        '''
            elif re.search('fex', feature , re.I):
                sw_cmd = '''install feature-set fex
                           feature-set fex'''
            elif re.search('nve',feature,re.I):
                sw_cmd='''feature nv overlay
                        '''
            elif re.search('nv_overlay_evpn',feature,re.I):
                    hdl.iconfig('nv overlay evpn')
                    continue
            else:
                    sw_cmd='''feature {0}
                               '''.format(feature)
        ...
        for feature in str.split(feature_name, ','):
            ...
            log.info('The value of feature after mapping  is : %s' % feature)
            log.info('The value of tmp_feature after mapping  is : %s' % tmp_feature)
            if not re.search('fex', feature, re.I):
...
class  unconfigFeature():
    ...
    def __init__(self, hdl, log, *args):
        ...
        FeatureMapping = { 'telnet':'telnetServer', 
                           ...
                           'vn-segment-vlan-based':'vnseg_vlan' }
        for feature in str.split(feature_name, ','):
            log.info('The value of feature  is : %s' % feature)
            if re.search('hsrp',feature,re.I):
                ...
                sw_cmd='''no feature {0} 
                   '''.format(feature)
            hdl.configure(sw_cmd, timeout = 600)
            time.sleep(7)
        for feature in str.split(feature_name, ','):
