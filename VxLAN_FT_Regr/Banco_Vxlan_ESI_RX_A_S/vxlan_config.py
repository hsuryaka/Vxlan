...
from vxlan_config_cli_arg import change_vxlan_cli_arg
...
class InitializeTestbed(nxtest.CommonSetup):
    ...
    def connect_devices(self, testbed, testscript, steps, datafile_path, verify_file_path, user_new_cli_flag=False,
                        nx_cloud_flag= False,nx_cloud_datafile=None,nx_cloud_datafile_local=None,
                        datafile_path_new_cli=None, verify_file_path_new_cli=None, cores_check_flag=False,enable_gnmi_connection=False, **kwargs):
        """
            Establish connection to all testbed devices and collect all script parameters
                ...
                Return:
                     None
        """
        cutom_run_time_args = runtime.env.argv.get('custom', None)
        testscript.parameters['cutom_run_time_args'] = cutom_run_time_args
        with steps.start("Initializing the environment for Genie"):
            ...
            new_l3_vni_min_version = kwargs.get('new_l3_vni_min_version', '10.2(2)')
            testsuite_support_min_version = kwargs.get('testsuite_support_min_version', '10.3(1.0)')
            trmv6_eor_min_version_support = kwargs.get('trmv6_eor_min_version_support', False)
            ver_pattern = "(.*?)\((.*?)\)"
            match = re.search(ver_pattern, new_l3_vni_min_version)
            ...
            if match:
                ...
                if maj_match:
                    if not maj_match.groups()[1]:
                        major_version_default = float(maj_match.groups()[0])
                    else:
                        major_version_default = float(".".join(maj_match.groups()))
                min_match = re.search(pattern, match.groups()[1])
                if min_match:
                    if not min_match.groups()[1]:
                        minor_version_default = float(min_match.groups()[0])
                    else:
                        minor_version_default = float(".".join(min_match.groups()))

            match = re.search(ver_pattern, testsuite_support_min_version)
            if match:
                pattern = "([0-9]+)\.?([0-9]+)?"
                maj_match = re.search(pattern, match.groups()[0])
                if maj_match:
                    if not maj_match.groups()[1]:
                        testsuite_major_version_default = float(maj_match.groups()[0])
                    else:
                        testsuite_major_version_default = float(".".join(maj_match.groups()))
                min_match = re.search(pattern, match.groups()[1])
                if min_match:
                    if not min_match.groups()[1]:
                        testsuite_minor_version_default = float(min_match.groups()[0])
                    else:
                        testsuite_minor_version_default = float(".".join(min_match.groups()))

            if trmv6_eor_min_version_support:
                match = re.search(ver_pattern, trmv6_eor_min_version_support)
                if match:
                    pattern = "([0-9]+)\.?([0-9]+)?"
                    maj_match = re.search(pattern, match.groups()[0])
                    if maj_match:
                        if not maj_match.groups()[1]:
                            trmv6_eor_major_version_default = float(maj_match.groups()[0])
                        else:
                            trmv6_eor_major_version_default = float(".".join(maj_match.groups()))
                    min_match = re.search(pattern, match.groups()[1])
                    if min_match:
                        if not min_match.groups()[1]:
                            trmv6_eor_minor_version_default = float(min_match.groups()[0])
                        else:
                            trmv6_eor_minor_version_default = float(".".join(min_match.groups()))

            LOG.info('New l3 vni major version: %s', major_version_default)
            LOG.info('New l3 vni minor version: %s', minor_version_default)
            LOG.info('Testsuite major version: %s', testsuite_major_version_default)
            LOG.info('Testsuite minor version: %s', testsuite_minor_version_default)
        ...
        testscript.parameters["eor"] = False
        testbed.custom["eor"] = False
        testbed.custom["eor_version_check"] = False
        trm_eor = kwargs.get("trm_eor", False)
        ...
        for node in testscript.parameters['testbed'].find_devices(
            ...
            with steps.start(f"Verify switch version in {node.alias}"):
                ...
                if match:
                    ...
                    LOG.info("Minimum supported version for new l3 vni: %s", new_l3_vni_min_version)
                    LOG.info("Minimum supported version for testsuite: %s", testsuite_support_min_version)
                    LOG.info("Switch version: %s(%s)", major_version,minor_version)

                    if major_version < testsuite_major_version_default:
                        LOG.error("Please upgrade all the DUTs to the supported version. The testsuite is not supported in version: %s(%s)", major_version, minor_version)
                        self.failed()
                    if major_version == testsuite_major_version_default:
                        if minor_version < testsuite_minor_version_default:
                            LOG.error("Please upgrade all the DUTs to the supported version. The testsuite is not supported in version: %s(%s)", major_version, minor_version)
                            self.failed()
                ...
                '''Getting Device is EOR or TOR to load required verify and datafile'''
                obj = ShowModule(device=node)
                mod_dict = obj.parse()
                if mod_dict['slot'].get('lc'):
                    rp = list(mod_dict.get('slot').get('rp').keys())
                    vsup = [x for x in rp if mod_dict.get('slot').get('rp').get(x).get('Virtual Supervisor Module')]
                    if not vsup:
                        testscript.parameters["eor"] = True
                        testbed.custom["eor"] = True
                        LOG.info("%s is EOR ",node)
                        if trmv6_eor_min_version_support:
                            if major_version < trmv6_eor_major_version_default:
                                LOG.info("TRM EOR minimum supported version %s(%s)",trmv6_eor_major_version_default, trmv6_eor_minor_version_default)
                                LOG.info("%s current version is  %s(%s)", node,  major_version, minor_version)
                                testbed.custom["eor_version_check"] = True
                            if major_version == trmv6_eor_major_version_default:
                                if minor_version < trmv6_eor_minor_version_default:
                                    LOG.info("TRM EOR minimum supported version %s(%s)",trmv6_eor_major_version_default, trmv6_eor_minor_version_default)
                                    LOG.info("%s current version is  %s(%s)", node,  major_version, minor_version)
                                    testbed.custom["eor_version_check"] = True

            with steps.start(f"Verify core file in {node.alias}"):
                if cores_check_flag:
                    global copy_cores
                    copy_cores = True
        ...
        if user_new_cli_flag and all(flag == True for flag in ver_dict.values()) and not testbed.custom["ixia_flag"]:
            ...
            # Verification of EOR devices or not and setting required new L3 vni cli verify and data file based on EOR
            if trm_eor and testbed.custom["eor"] and testbed.custom["eor_version_check"]:
                verify_file_path_new_cli = kwargs.get("verify_file_path_new_cli_eor", verify_file_path_new_cli)
                datafile_path_new_cli = kwargs.get("datafile_path_new_cli_eor", datafile_path_new_cli)
            with steps.start("Load New L3 VNI verify yaml file with Image version {maj_version}({min_version})".format(
            ...
            with steps.start("Load New L3 VNI config yaml file with Image version {maj_version}({min_version})".format(
                    maj_version=major_version, min_version=int(minor_version))):
            ...
            with steps.start("Load verify yaml file with Image version {maj_version}({min_version})".format(
                    maj_version=major_version, min_version=int(minor_version))):
                # Verification of EOR devices or not and setting required verify and data file based on EOR
                if trm_eor and testbed.custom["eor"] and testbed.custom["eor_version_check"]:
                    verify_file_path = kwargs.get("verify_file_path_eor", verify_file_path)
                    datafile_path = kwargs.get("datafile_path_eor", datafile_path)
                # Verification parameters for Control plane verification
                if verify_file_path:
    ...
    @aetest.subsection
    def change_config_for_vxlan_cli_arg(self, testbed, testscript, steps, **kwargs):
        change_vxlan_cli_arg(testbed, testscript, steps, **kwargs)
...
class CommonSetup(nxtest.CommonSetup):
    ...
    """
        Commenting this code since the model is not ready and config is coming from missing_config
    """
    # @aetest.subsection
...
class ConfigureWithTemplate(nxtest.CommonSetup):
                            ...
                            if 'interface breakout module' in contents[-1]: #Handling last line to prevent IndexError
                                ...
                                for index, line in enumerate(contents):
                                    if 'interface breakout module' in line and break_out_ports not in contents[index + 1]:
                                        ...
                                        contents.insert(index, break_out_ports)
                                        break
                            fh.seek(0)
    ...
    @aetest.subsection
    def change_cli_arg_config(self,testbed, testscript, steps, **kwargs):
        # change config with cli args
        ixia_cfg_change_param = kwargs
        ixia_cfg_change_param.update({'ixia_profile_cfg_change': True})
        change_vxlan_cli_arg(testbed, testscript, steps, **ixia_cfg_change_param)
