...
def main():
    ...
    testscript = os.path.join(test_path, '../pyats/VxLAN_MLAG_CloudSec/VxLAN_MLAG_CloudSec_ScriptFile.py')
    configurationFile = os.path.join(test_path, '../pyats/VxLAN_MLAG_CloudSec/VxLAN_MLAG_CloudSec_ConfigFile.yaml')
    ...
    uut_devices = {
        'BGW-1'                 : 'BGW-1',
        'BGW-2'                 : 'BGW-2',
        'BGW-1-Site-2'          : 'BGW-1-Site-2',
        'MLAG-L2-Access-SW-1'   : 'MLAG-L2-Access-SW-1',
        'Spine-1-Site-1'        : 'Spine-1-Site-1',
        'VPC-Node-1'            : 'VPC-Node-1',
        'VPC-Node-2'            : 'VPC-Node-2',
        'Leaf-01-Site-1'        : 'Leaf-01-Site-1',
        'L2-VPC-Acc-SW-1'       : 'L2-VPC-Acc-SW-1',
        'DCI-Route-Server-1'    : 'DCI-Route-Server-1',
        'Spine-1-Site-2'        : 'Spine-1-Site-2',
        'Leaf-02-Site-2'        : 'Leaf-02-Site-2',
        'ixia'                  : 'IXIA',
    }
   ...
   # run(testscript=testscript, uut_list=uut_devices, configurationFile=configurationFile, job_file_params=jobFileParams, uids=Or('common_setup', 'DEVICE_BRINGUP_.*', 'IXIA.*', 'VERIFY_NETWORK', 'TC013_.*' ))
    #run(testscript=testscript, uut_list=uut_devices, configurationFile=configurationFile, job_file_params=jobFileParams, uids=Or('common_setup', 'DEVICE_BRINGUP_.*', 'IXIA.*', 'VERIFY_NETWORK', 'TC007_.*', 'TC008_.*', 'TC011_.*', 'TC012_.*' ))
    #run(testscript=testscript, uut_list=uut_devices, configurationFile=configurationFile, job_file_params=jobFileParams, uids=Or('common_setup', 'DEVICE_BRINGUP_.*', 'IXIA.*', 'VERIFY_NETWORK','TC013_.*' ))
    run(testscript=testscript, uut_list=uut_devices, configurationFile=configurationFile, job_file_params=jobFileParams, uids=Or('common_setup', 'DEVICE_BRINGUP_.*', 'IXIA.*', 'VERIFY_NETWORK', 'TC.*' ))
