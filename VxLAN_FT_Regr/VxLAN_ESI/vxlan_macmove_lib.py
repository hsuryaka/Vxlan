...
def feature_enable(uut,feature_list):
    ...
    for line in op1:
        if line:
            ...
            if not cmd in ['telnet','nxapi','bash-shell','scp-server','sftp-server','fex']:
...
class VPCNode(object):
    ...
    def vpc_conf(self):
        ...
        '''
        try:
            self.node.configure(cmd.format(peer_ip=self.peer_ip,vpc_domain=self.vpc_domain))
        except:
            log.error('111 vpc gloabal config failed')
...
def SpirentTunnelStreamConf(port_handle,source_mac,protocol):
...
def CreateSpirentStreams2(port_hdl,ip_src,ip_dst,mac_src,mac_dst,stream_id,rate_pps,smac_count,dmac_count,dmac_step,dmac_mode,smac_mode,transmit_mode,smac_step,vlan_id):
    #self,port_hdl_list,ip_src_list,ip_dst_list,mac_src_list,mac_dst_list,stream_list,rate_pps,mac_count,mac_mode,mac_step,vlan_id): 
    """ function to configure Stream """
    logger.info(banner("Entering proc to configure streams in Spirent"))
    try:
...
def VxlanMacMoveTest(port_hdl1,port_hdl2,\
                     ip_src1,ip_dst1,mac_src1,mac_dst1,stream_id1,\
                     ip_src2,ip_dst2,mac_src2,mac_dst2,stream_id2,\
                     vtep_uut_list,sw1,sw1_intf1,sw1_intf2,\
                     rate_pps,mac_count,mac_mode,mac_step,vlan_id):

    logger.info(banner("Entering proc to configure streams in Spirent"))
    streamblock_ret1 = sth.traffic_config ( mode = 'create', port_handle = port_hdl1,\
            l2_encap = 'ethernet_ii_vlan',frame_size_min='500',frame_size_max='9000',frame_size_step='500',rate_pps = '10000',\
            vlan_id= vlan,l3_protocol = 'ipv4',mac_discovery_gw = ip_gw,\
            mac_src = '58:58:aa:aa:00:01',mac_src_count = 1 ,mac_src_mode='increment',mac_src_step='1',\
            ip_src_addr = ip1,ip_src_count = 1 ,ip_src_mode = 'increment',ip_src_step ='0.0.0.1',\
            ip_dst_addr = ip2,ip_dst_count = 1 ,ip_dst_mode = 'increment',ip_dst_step ='0.0.0.1')


    streamblock_ret1 = sth.traffic_config ( mode = 'create', port_handle = port_hdl2,\
            l2_encap = 'ethernet_ii_vlan',frame_size_min='500',frame_size_max='9000',frame_size_step='500',rate_pps = '10000',\
            vlan_id= vlan,l3_protocol = 'ipv4',mac_discovery_gw = ip_gw,\
            mac_src = '58:58:bb:bb:00:01',mac_src_count = 1 ,mac_src_mode='increment',mac_src_step='1',\
            ip_src_addr = ip2,ip_src_count = 1 ,ip_src_mode = 'increment',ip_src_step ='0.0.0.1',\
            ip_dst_addr = ip1,ip_dst_count = 1 ,ip_dst_mode = 'increment',ip_dst_step ='0.0.0.1')
    ...
    for port_hdl in [port_hdl1,port_hdl2]:
        ...
        traffic_ctrl_ret = sth.traffic_control(port_handle = port_hdl, action = 'run',traffic_start_mode = 'sync',  duration = '30')
        traffic_ctrl_ret = sth.traffic_control (port_handle = port_hdl, action = 'stop', db_file=0 ) 
    ...
    for port_hdl in [port_handle_sw1,port_handle_sw3]:
        traffic_ctrl_ret = sth.traffic_control (port_handle = port_hdl, action = 'stop', db_file=0 ) 
        traffic_ctrl_ret = sth.traffic_control(port_handle = port_hdl, action = 'run',traffic_start_mode = 'sync',  duration = '10')
        traffic_ctrl_ret = sth.traffic_control (port_handle = port_hdl, action = 'stop', db_file=0 ) 

    res1 = sth.traffic_stats(port_handle = port_hdl, mode = 'streams') 
    log.info('Traffic status on Spirent Port Hdl %r is %r',port_hdl,res1)
            #log.info('DA/SA good, srcip_subnet is %r,dstip_subnet is %r',srcip_subnet,dstip_subnet)
    ...
    for stream_id in stream_list:    
        ...
        if rx_count != tx_count:
            ...
            self.failed()
    mactcount1a = uut1.execute("show mac address-ta dynamic vlan {vlan} | incl 'aa:aa' | count".format(vlan=vlan) )
    mactcount1b = uut1.execute("show mac address-ta dynamic vlan {vlan} | incl 'bb:bb' | count".format(vlan=vlan) )
    arpcount1a = uut1.execute("show ip arp vrf {vrf} | incl 'aa:aa' | count".format(vrf=vrf) )
    arpcount1b = uut1.execute("show ip arp vrf {vrf} | incl 'bb:bb' | count".format(vrf=vrf) )
...
def ConvergenceTest(port_hdl1,port_hdl2,rate_fps):
    log.info(banner("    *    *   *    *Starting the function ConvergenceTest    *    *   *    *"))
    for port_hdl in [port_hdl1,port_hdl2]:  
        ...
        if abs(int(rx_rate) - int(tx_rate)) > 50:
            log.info('Traffic  Rate Test failed')
    ...
    log.info(banner(" Stopping the Traffic in the function ConvergenceTest"))
    for port_hdl in [port_hdl1,port_hdl2]:
        traffic_ctrl_ret = sth.traffic_control (port_handle = port_hdl, action = 'stop', db_file=0 ) 
    countdown(20)  


    res1 = sth.drv_stats(query_from = port_hdl1,properties = "Port.TxTotalFrameCount Port.RxTotalFrameCount")
    res2 = sth.drv_stats(query_from = port_hdl2,properties = "Port.TxTotalFrameCount Port.RxTotalFrameCount")

    res1_tx_count = res1['item0']['PortTxTotalFrameCount']    
    res1_rx_count = res1['item0']['PortRxTotalFrameCount']
    res2_tx_count = res2['item0']['PortTxTotalFrameCount']    
    res2_rx_count = res2['item0']['PortRxTotalFrameCount']

    drop_1_2 = abs(int(res1_tx_count) - int(res2_rx_count))
    drop_2_1 = abs(int(res2_tx_count) - int(res1_rx_count))

    drop_1_2_seconds  = drop_1_2/rate_fps
    drop_2_1_seconds  = drop_2_1/rate_fps

    log.info('+-----------------------------------------------------------------------+')
    log.info('drop_1_2 is %r,drop_2_1 is %r',drop_1_2,drop_2_1)
    log.info('+-----------------------------------------------------------------------+')
    log.info('+-----------------------------------------------------------------------+')
    log.info('Convergence 1 to 2  in seconds is %r,Convergence 2 to 1  in seconds is %r',drop_1_2_seconds,drop_2_1_seconds)
    log.info('+-----------------------------------------------------------------------+')
        
    if drop_1_2_seconds > 50:
        log.info('Traffic  Rate Test failed')
        log.info('Streamblock is %r',res)
        for port_hdl in [port_hdl1,port_hdl2]:
...
def SpirentRateTest(port_hdl1,port_hdl2,rate_fps):
    log.info(banner("   *  Starting the function SpirentRateTest  *"))
    ...
    for port_hdl in [port_hdl1,port_hdl2]:
        ...
        if abs(int(rx_rate) - int(rate_fps)) > 50:
            log.info('Traffic  Rate Test failed')
            log.info('Streamblock is %r',res)
            return 0
...
def SpirentRateTest2(port_hdl1,port_hdl2,rate_fps,diff):
    log.info(banner("   *  Starting the function SpirentRateTest  *"))
...
def SpirentFrameCountTest(port_hdl1,port_hdl2,peer_scale):
    for port_hdl in [port_hdl1,port_hdl2]:
        traffic_ctrl_ret = sth.traffic_control (port_handle = port_hdl, action = 'stop', db_file=0 ) 
        sth.traffic_control(port_handle = port_hdl, action = 'clear_stats')
        traffic_ctrl_ret = sth.traffic_control(port_handle = port_hdl, action = 'run')
        ...
        for stream_id in stream_list:    
            ...
            if abs(int(rx_count) - int(peer_scale)*int(tx_count)) > 100:
                log.info('Traffic Test failed')
