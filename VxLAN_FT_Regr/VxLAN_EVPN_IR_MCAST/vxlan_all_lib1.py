            ...
            if 'vn-segment' in line:
                ...
                if not 'arp' in line:
                    if not 'vlan configuration' in line:
                        cmd += line + '\n'
                    elif not 'ip igmp snooping querier' in line:
                        cmd += line + '\n'
...
def arp_supp_remove_final(uut):
    ...
    op = uut.execute('show run int nve1 | beg nve1 | no')
    op1 = op.splitlines()
    cmd = " "
    for line in op1:
        if line:
...
def swPoConfigure(uut,vlan_range):
    ...
    uut.configure("spanning-tree mode mst")
...
def SwPortChannelconfigsfull(uut,po_number,port_list,vlan_range):
    ...
    if not 'none' in vlan_range:
        # Removing STP edge configs as the FAN OUT does not need these
        # cmd = """\
