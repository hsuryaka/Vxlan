...
def reload_all_vxlan_reset(uut_list):
    log.info(banner("starting reload_all"))
    op = pcall(Asciireloaduut,uut=(uut_list))
    countdown(600)
    return 1
...
def VxlanStReset(uut_list):
    ...
    for uut in uut_list:
        if 'isis' in uut.execute('show ip route'):
            igp = 'isis'
        else:
            igp = 'ospf'
        L3InterfaceFlap(uut,igp)   
...
def ConfigureL3PortvxlanMultisite(uut):
    ...
    for intf in l3_unnumbered_intf_list:
        ...
        eth.shutdown = False;eth.mtu = 1500;eth.medium = 'p2p';eth.unnumbered_intf_ref = 'loopback1'
    ...
    for intf in [*uut.interfaces.keys()]:
        if 'dci' in intf:
            ...
            eth.shutdown = False;eth.mtu = 1500; eth.ipv4 = ip_add1
...
def Singlel3pospineleaf(uut):       
    ...
    for intf in uut.interfaces.keys():
        if 'l3_single_po' in uut.interfaces[intf].type:
            ...
            for intf in po_member_list:
                log.info("Intf is ^^^^^^^ %r",intf)
                member = str(uut.interfaces[intf].alias)
                log.info("member is ^^^^^^^ %r",member)
                #if member.strip("Link leaf_spine_") == Po:
                log.info("interface is ^^^^^^^ %r", uut.interfaces[intf].intf)
                po_mem_list.append(uut.interfaces[intf].intf)
            log.info('l3 po mem list for po %r uut %r is %r',Po,str(uut),po_mem_list)
            uut_l3Po_obj = CLI_PortChannel(uut,Po,'Nil','layer3',po_mem_list,ipv4_add)
            uut_l3Po_obj.ConfigurePo()
...
def XconnectRateTest2(orph_port_handle_list):
    ...
    for port_hdl in orph_port_handle_list:        
        ...
        try:               
            ...
            log.info('rx_rate0 for port %r  is %r',port_hdl,rx_rate0)
        except:
           log.error('rx_rate failed for port %r',port_hdl)  
           return 0     
                
        if abs(int(rx_rate0) - int(4000)) > 400: 
            log.info('Traffic Test FAIL  orph_port_handle_list-  ,port_hdl %r',port_hdl)
            result_list.append('fail')
...
def config_to_bootflash1(uut,filename):
    #hostname = uut.execute('show hostname')
...
def reload_with_valid_config1(uut,filename):
    log.info(banner('------ in reloadUUT ------ '))
    uut.transmit('write erase\n')
    uut.transmit("y\n")
    countdown(1)
