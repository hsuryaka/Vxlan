...
class configureInterfaces(object):
    ...
    def __init__(self,log, switch_hdl_dict=None, interface_config_dict=None):
        ...
        for node in self.interface_config_dict.keys():
           ...
           log.info('Inside the node clause.. Value of  self.interface_config_dict is : {0}'.format(self.interface_config_dict))
           log.info('Inside the node clause.. and the value of node is : {0}'.format(node))
           if 'ethernet' in self.interface_config_dict[node].keys():
              log.info('Inside the ethernet clause')
              for intf in self.interface_config_dict[node]['ethernet'].keys():
                 ...
                 self.hdl_dict[node].configure(cfg, timeout = 300)
           ...
           if 'portchannel' in self.interface_config_dict[node].keys():
              for intf in self.interface_config_dict[node]['portchannel'].keys():
                 ...
                 self.hdl_dict[node].configure(cfg, timeout = 300)
                 self.hdl_dict[node].configure('feature lacp', timeout = 300)
    ...
    def configureSvi(self, node):
        ...
        sw_hdl=self.hdl_dict[node]
        sw_hdl.configure('feature interface-vlan', timeout = 300)
...
class configInterface(object):
    ...
    def __init__(self,log,switch_hdl_dict=None,interface_config_dict=None,*args,**kwargs):
        ...
        self.log=log
        self.log.info('inside __init__ function of configInterface')
        self.log.info('inside __init__ function. Value of switch_hdl_dict is %s ' % switch_hdl_dict)
        self.log.info('inside __init__ function. Value of interface_config_dict is %s ' % interface_config_dict)
        self.result='pass'
        ...
        self.log.info('inside __init__ function. Value of keyList is %s ' % keyList)
    ...
    def configureVrfEvpn(self):
        ...
        self.log.info('The value of self.interface_Config_dict.keys() is: {0}'.format(self.interface_config_dict.keys()))
        vrf_dict=self.interface_config_dict['vrf']
        self.log.info('\nvrf_dict : {0}'.format(vrf_dict))
        self.log.info('\n self.hdl_dict : {0}'.format(self.hdl_dict))
        self.log.info('\n self.hdl_dict.keys : {0}'.format(self.hdl_dict.keys()))
        for sw in  self.hdl_dict.keys():
            sw_hdl=self.hdl_dict[sw]
            self.log.info('\n sw_hdl : {0}'.format(sw_hdl))
            ...
            if re.findall('(type \'dict\')',str(type(sw_vrf_dict))):
             for vrf in sw_vrf_dict.keys():
                     if 'evpn' in sw_vrf_dict[vrf].keys():
                             ...
                             out=sw_hdl.configure(cfg, timeout = 300)
                             if out:
    ...
    def configureVrf(self):
        ...
        for sw in  self.hdl_dict.keys():
            ...
            self.log.info('\n sw_vrf_dict : {0}'.format(sw_vrf_dict))
            self.log.info('\n Type of sw_vrf_dict : {0}'.format(type(sw_vrf_dict)))
            self.log.info('\n Type of str\(sw_vrf_dict\) : {0}'.format(str(type(sw_vrf_dict))))
            if re.findall('(class \'dict\')',str(type(sw_vrf_dict))):
             for vrf in sw_vrf_dict.keys():
              cfg+='vrf context {0}\n'.format(vrf)
              ...
              if sw_add_dict:
                if 'address_family' in sw_add_dict.keys():
                   ...
                   #self.log.info('\n  add_family_dict: {0}'.format(add_family_dict()))
                   #self.log.info('\n  add_family_dict.keys(): {0}'.format(add_family_dict.keys()))  
                   for var in add_family_dict.keys():
                      self.log.info('\n  value of var is : {0}'.format(var))
                      bo_args=add_family_dict[var]
                      self.log.info('\n  value of bo_args is : {0}'.format(bo_args))
                      if not configureVrfContextAddressFamily(sw_hdl,var,vrf,bo_args,self.log):
                         #self.log.error('Address family config on vrf failed on {0} ',format(sw_hdl.name))
                         return 0  
            else: 
    ...
    def configureSvi(self):
        ...
        self.log.info('VAlue of sVI_DICT in ConfigureSVI  %s' % svi_dict)
        for sw in  self.hdl_dict.keys():
            sw_hdl=self.hdl_dict[sw]
            if sw in svi_dict.keys():
                 self.log.info('VAlue of sVI_DICT.keys in ConfigureSVI  %s' % sw)
                 sw_svi_dict=svi_dict[sw]
                 self.log.info('VAlue of sw_svi_dict in ConfigureSVI  %s' % sw_svi_dict)
                 for var in sw_svi_dict.keys():
                     self.log.info('VAlue of var in ConfigureSVI  %s' % var)
                     svi_args=sw_svi_dict[var]
                     if not configureSviIntf(sw_hdl,var,svi_args,self.log):
                         #self.log.error('Configuring SVI interface failed on {0} with {1}'.format(sw_hdl.name,svi_args))
                         return 0
...
def configurepvlan(sw_hdl,pvlan,pvlan_args,log):
    ...
    sw_hdl.configure(cfg,timeout = 300)
    if sw_hdl.errFlag:
        return 0
    return 1
...
def configureVrfContextAddressFamily(sw_hdl,add_family,vrf_name,vrf_args,log):
      ...
      arggrammar['trm']='-type bool'
      arggrammar['vni']='-type int'
      cfg=''
      try:
...
def configureL3Intf(sw_hdl,intf,intf_args,log):
    ...
    arggrammar['load_interval_1']='-type int -default 5'
    arggrammar['load_interval_2']='-type int -default 10'
    arggrammar['load_interval_3']='-type int -default 15' 
    ...
    if ns.vrf:
        cfg+='vrf member {0}\n'.format(ns.vrf)
    if ns.load_interval_1:
        pass
        #cfg+='#load-interval counter 1 {0}\n'.format(ns.load_interval_1)
    if ns.load_interval_2:
        pass
        #cfg+='#load-interval counter 2 {0}\n'.format(ns.load_interval_2)
    if ns.load_interval_3:
        pass
        #cfg+='#load-interval counter 3 {0}\n'.format(ns.load_interval_3) 
    if ns.encapDot1q:
    ...
    if ns.ipv4_addr_sec and ns.ipv4_prf_len_sec:
        if ns.tag:
            cfg+='ip address {0}/{1} secondary tag {2}\n'.format(ns.ipv4_addr_sec,ns.ipv4_prf_len_sec,ns.tag)
        else:
            cfg+='ip address {0}/{1} secondary \n'.format(ns.ipv4_addr_sec,ns.ipv4_prf_len_sec)  
...
def configureL2Intf(sw_hdl,intf,intf_args,log):
    ...
    arggrammar['spanning_tree_edge_trunk']='-type bool -default False'
    arggrammar['allowed_vlan_list']='-type str'
    arggrammar['fex_associate']='-type str'
    arggrammar['speed']='-type str'
    ...
    if ns.switchportmode=='fex_fabric' :
        cfg+='''switchport mode fex-fabric
             '''
        if ns.fex_associate:
                cfg+='''fex associate {0}
    ...
    if ns.spanning_tree_edge_trunk:
...
def configurePomemberPorts(sw_hdl,intf,intf_args,log):
    ...
    kdict={}
    kdict['verifySuccess']=True
    sw_hdl.configure(cfg,timeout = 300,**kdict)
    #if sw_hdl.errFlag:
    #    return 0
    return 1
...
class unconfigInterface(object):
    ...
    def unconfigureSvi(self):
        ...
        for sw in svi_dict.keys():
            ...
            sw_svi_dict=svi_dict[sw]
            for var in sw_svi_dict.keys():
                out = sw_hdl.configure('no interface vlan {0}'.format(var), timeout = 300)
                if out != None:
                  result = 0
        return result
        ...
        for sw in po_dict.keys():
            ...
            for var in sw_po_dict.keys():
                ...
                while i == 0 :
                  ...
                  time.sleep(30)
                  out = sw_hdl.configure('no interface {0}'.format(var),verifySuccess=True, timeout = 300)
    ...
    def unconfigureLoopback(self):
        ...
        for sw in lo_dict.keys():
            ...
            sw_lo_dict=lo_dict[sw]
            for var in sw_lo_dict.keys():
                out = sw_hdl.configure('no interface {0}'.format(var), timeout = 300)
                if out != None:
                  result = 0
        return result
        ...
        for sw in eth_dict.keys():
            ...
            for var in sw_eth_dict.keys():
                ...
                if var == 'breakout_ports':
                    ...
                    out = sw_hdl.configure('default interface {0}'.format(var), timeout = 300)
                    if out != None:
                      result = 0
        return result
        ...
        if 'members' in params_list and re.search('[pP]o[0-9]+',intf):
            ...
            if not verify_mode:
                ...
                if re.search('Eth[0-9]{3}/[0-9]/[0-9]+',normalizeInterfaceName(self.log,member_intf)):
                    ...
                    if not self.generate_ascii_only:
                        hdl.configure(cmds, timeout = 300)
    ...
    interface_config_dict:
        node01:
            ...
            Vlan2-200: 
                ...
                ipv4: 
                    -start_ipv4_addr 10.1.2.252 -start_ipv4_prf_len 24 -step_'-dut {0}'.format(dut) 0.0.1.0 -vrf_name default
            all:
                port_profile: 
                    -name profile1 # This will ignore the rest of the parameters
                base_config: 
...
def parseIpv4Configs( log, args):
    ipv4_grammar={}
    # ipv4 config
    ipv4_grammar['ipv4_addr']='-type str -mandatoryargs ipv4_prf_len -format {0}'.format(utils.rex.IPv4_ADDR)
    ipv4_grammar['ipv4_prf_len']='-type str -mandatoryargs ipv4_addr -format [0-9]+'
    ipv4_grammar['secondary_ipv4']='-type list -mandatoryargs ipv4_addr' #Format: [('1.2.3.4',24),('192.168.1.1',24)]
    ipv4_grammar['vrf']='-type str -default default'
    ipv4_grammar['proxy_arp']='-type bool'
