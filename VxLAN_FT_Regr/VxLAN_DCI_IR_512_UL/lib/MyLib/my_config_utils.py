...
def parseLoopbackParams(log,args):
    ...
    arggrammar['pim_enable'] = '-type bool'
    arggrammar['pim6_enable'] = '-type bool'
    arggrammar['ospf_enable'] = '-type bool'
    ns = utils.parserutils_lib.argsToCommandOptions(args,arggrammar,log)
    return ns
...
def cfgLoopbackIntf(dut,hdl,loopback_config_dict,log):
    ...
    if ns.ipv6_addr_start:
        ipv6_addr_list = ipv6addrgen(ns.no_of_loopback,ns.ipv6_addr_start,ns.ipv6_addr_mask)
    ...
    for i in range(ns.no_of_l2_vni):
        if hasattr(ns, 'evpn_ir') and ns.evpn_ir:
            ...
            if hasattr(ns, 'l2_vni_mcast') and ns.l2_vni_mcast:
                ...
                cfg = '''interface nve1
                         member vni {0}
                         mcast-group {1}'''.format(int(ns.l2_vni_start+i),mcast_grp_list[j]) + '\n'
        ...
        if hasattr(ns, 'evpn_mcast') and ns.evpn_mcast:
            if hasattr(ns, 'trm_mcast_group_start') and ns.trm_mcast_group_start:
                ...
                cfg = '''interface nve 1
                         ...
                         mcast-group {1}'''.format(int(ns.l3_vni_start+i),trm_mcast_group_list[j]) + '\n'
                if ((ns.l2_vni_start + i) % ns.mcast_grp_per_vni == 0):
                    j+=1
        hdl.configure(cfg)
    return 1
...
def parseScaleVRFParams(log,args):
    ...
    arggrammar['pim6_rp_addr_start'] = '-type str'
    arggrammar['pim6_rp_addr_mask'] = '-type int '
    arggrammar['pim6_group_list'] = '-type str'
    arggrammar['rd'] = '-type str'
...
def configureVRFs(log,hdl,args):
    ...
    if ns.pim6_rp_addr_start:
        pim6_rp_addr_list = ipv6addrgen(ns.no_of_vrf,ns.pim6_rp_addr_start,ns.pim6_rp_addr_mask)
        log.info('The vale of pim6_rp_addr_list is: {0}'.format(pim6_rp_addr_list))
    for i in range(0,ns.no_of_vrf):
        cfg = ''
        ...
        if hasattr (ns,'pim_rp_addr_start') and ns.pim_rp_addr_start:
            ...
            if hasattr(ns,'pim_group_list') and ns.pim_group_list:
                ...
                cfg += '\n'
        if hasattr (ns,'pim6_rp_addr_start') and ns.pim6_rp_addr_start:
            cfg += 'ipv6 pim rp-address '+ pim6_rp_addr_list[i] + ' '
            if hasattr(ns,'pim6_group_list') and ns.pim6_group_list:
                cfg += 'group-list ' + ns.pim6_group_list + '\n'
            else:
                cfg += '\n'
        cfg += 'ip multicast multipath s-g-hash next-hop-based \n' 
        cfg += 'ipv6 multicast multipath sg-nexthop-hash \n'
