...
def main():
    ...
    test_path.append('n9k_vmct_mcast_local.py')
    testscript = '/'.join(test_path)
    # run api launches a testscript as an individual task.
    #run(testscript)
    # to run specific testcase
    run(testscript,uids =Or('common_setup','setupConfigVxlan','setupTgen','VXLAN-VMCT-MCAST-FUNC-019'))
    #run(testscript,uids =Or('common_setup','VXLAN-VMCT-MCAST-FUNC-019'))
